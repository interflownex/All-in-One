...
def _load_api_hub(monkeypatch):
    ...
    monkeypatch.setenv(
        "ALL_IN_ONE_API_KEYS",
        "test-key:test-client:gateway:read;denied-key:denied:jobs:read",
    )
    ...
    spec = importlib.util.spec_from_file_location(
        "all_in_one_api_hub_gateway_security", path
    )
...
def test_api_key_check_rejects_missing_invalid_and_unscoped_keys(monkeypatch):
    ...
    assert (
        client.get("/gateway/api-key/check", headers={"X-API-Key": "wrong"}).status_code
        == 401
    )
    assert (
        client.get(
            "/gateway/api-key/check", headers={"X-API-Key": "denied-key"}
        ).status_code
        == 403
    )
...
def test_webhook_signature_verification(monkeypatch):
    ...
    accepted = client.post(
        ...
        content=body,
        headers={
            "X-All-In-One-Signature": f"sha256={digest}",
    ...
    rejected = client.post(
        ...
        content=body,
        headers={
            "X-All-In-One-Signature": "sha256=invalid",
            "Content-Type": "application/json",
        },
    )
...
def test_rate_limiter_blocks_after_limit(monkeypatch):
    ...
    try:
        ...
        raise AssertionError(
            "rate_limiter deveria bloquear quando limite estiver esgotado"
        )
...
def test_health_module_proxy_paths_still_require_jwt(monkeypatch):
    ...
    public_health = Request(
        {"type": "http", "method": "GET", "path": "/health", "headers": []}, receive
    )
    ...
    assert (
        asyncio.run(module.validate_jwt_edge(module_health))["sub"]
