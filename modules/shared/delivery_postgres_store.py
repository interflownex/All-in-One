...
class DeliveryPostgresStore:
    ...
    def create(
        ...
        try:
            with self.transaction() as connection:
                ...
                if item is None:
                    raise RuntimeError(
                        "PostgreSQL nao retornou recurso Delivery criado."
                    )
                self._audit(
                    connection,
                    actor,
        ...
        if resource_type == "delivery_requests":
            return connection.execute(
                ...
                (
                    resource_id,
                    user_id,
                    payload.get("assigned_rider_user_id"),
                    ...
                    payload.get("quoted_brl"),
                    payload.get("insurance_required", False),
                    status,
                    metadata,
                    actor,
                    actor,
        ...
        if resource_type == "rider_profiles":
            return connection.execute(
                ...
                (
                    ...
                    user_id,
                    payload["wallet_id"],
                    payload.get("cnh_number_hash"),
                    payload.get("cnh_category"),
                    status,
                    metadata,
                    actor,
                    actor,
    ...
    def update(
        ...
        with self.transaction() as connection:
            ...
            if updated is None:
                raise RuntimeError(
                    "PostgreSQL nao retornou recurso Delivery atualizado."
                )
            self._audit(
                connection,
                actor,
    ...
    def metrics(self) -> tuple[int, int, int]:
        ...
        audits = self.connection.execute(
            "SELECT COUNT(*) AS count FROM audit.logs WHERE module = 'delivery'"
        ).fetchone()["count"]
