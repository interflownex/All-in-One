...
PUBLIC_LOCATION_FIELDS = frozenset(
    {"latitude", "longitude", "service_origin", "service_radius_km"}
)
...
RECRUITER_ROLES = frozenset(
    {"owner", "administrator", "hr_manager", "recruiter", "auditor"}
)
...
PRIMARY_RESOURCE = {
    module: resources[0] for module, resources in MODULE_ENTITIES.items()
}
...
def review_flow(prefix: str) -> dict[str, Transition]:
    return {
        "submit": Transition(
            frozenset({"draft", "pending_documents"}),
            "pending_review",
            event=f"{prefix}.submitted",
        ),
        "approve": Transition(
            frozenset(
                {"draft", "pending_review", "pending_validation", "under_review"}
            ),
            "approved",
            APPROVER_ROLES,
            True,
        ...
        "suspend": Transition(
            frozenset({"approved", "active"}), "suspended", APPROVER_ROLES, True
        ),
    }
...
def lifecycle_flow(prefix: str) -> dict[str, Transition]:
    return {
        "submit": Transition(
            frozenset({"draft"}), "pending_review", event=f"{prefix}.submitted"
        ),
        "approve": Transition(
            frozenset({"draft", "pending_review"}), "approved", APPROVER_ROLES, True
        ),
        "cancel": Transition(
            frozenset({"draft", "pending_review", "approved", "active"}),
            "cancelled",
            event=f"{prefix}.cancelled",
        ),
        "complete": Transition(
            frozenset({"approved", "active", "in_progress"}),
            "completed",
            event=f"{prefix}.completed",
        ),
    }
...
RULE_OVERRIDES: dict[tuple[str, str], ResourceRule] = {
    ("identity", "users"): ResourceRule(
        ...
        transitions={
            ...
            "suspend": Transition(
                frozenset({"ACTIVE"}), "SUSPENDED", APPROVER_ROLES, True
            ),
            "block": Transition(
                frozenset({"PENDING_KYC", "ACTIVE", "SUSPENDED"}),
                "BLOCKED",
                APPROVER_ROLES,
                True,
            ),
    ...
    ("identity", "sessions"): ResourceRule(
        ...
        transitions={
            "revoke": Transition(
                frozenset({"active"}), "revoked", event="identity.session.revoked"
            ),
        },
    ),
    ...
    ("finance", "wallets"): ResourceRule(
        ("wallet_type",), initial_status="active", sensitive=True
    ),
    ...
    ("finance", "escrows"): ResourceRule(
        ...
        transitions={
            "authorize": Transition(
                frozenset({"created"}), "authorized", APPROVER_ROLES, True
            ),
            "capture": Transition(
            ...
            "refund": Transition(
                frozenset({"authorized", "held", "disputed"}),
                "refunded",
                APPROVER_ROLES,
                True,
                "payment.refunded",
            ...
            "dispute": Transition(
                frozenset({"held"}), "disputed", event="payment.escrow.disputed"
            ),
        },
    ),
    ("finance", "splits"): ResourceRule(
        monetary_fields=("amount_brl",),
        sensitive=True,
        transitions=lifecycle_flow("finance.split"),
    ),
    ("finance", "invoices"): ResourceRule(
        monetary_fields=("amount_brl",),
        sensitive=True,
        transitions=lifecycle_flow("finance.invoice"),
    ),
    ...
    ("marketplace", "orders"): ResourceRule(
        ...
        transitions={
            "pay": Transition(
                frozenset({"created"}), "paid", event="marketplace.order.paid"
            ),
            "deliver": Transition(
                frozenset({"paid", "shipped"}),
                "delivered",
                event="marketplace.order.delivered",
            ),
            "cancel": Transition(
                frozenset({"created", "paid"}),
                "cancelled",
                event="marketplace.order.cancelled",
            ),
        },
    ),
    ...
    ("marketplace", "disputes"): ResourceRule(
        ...
        transitions={
            ...
            "close": Transition(
                frozenset({"open", "under_review", "resolved"}),
                "closed",
                APPROVER_ROLES,
                True,
    ...
    ("stock", "suppliers"): ResourceRule(
        ...
        initial_status="pending_validation",
        transitions=review_flow("stock.supplier"),
    ),
    ...
    ("delivery", "delivery_requests"): ResourceRule(
        ...
        transitions={
            "assign": Transition(
                frozenset({"created", "quoted"}),
                "assigned",
                event="delivery.rider.assigned",
            ),
            "pickup": Transition(
                frozenset({"assigned"}), "picked_up", event="delivery.picked_up"
            ),
            "complete": Transition(
                frozenset({"picked_up"}), "completed", event="delivery.completed"
            ),
            "cancel": Transition(
                frozenset({"created", "quoted", "assigned"}),
                "cancelled",
    ...
    ("riders", "vehicles"): ResourceRule(
        ("rider_profile_id", "type", "license_plate"),
        initial_status="pending_review",
        transitions=review_flow("rider.vehicle"),
    ),
    ("services", "providers"): ResourceRule(
        ("category",),
        initial_status="pending_review",
        protected_content=True,
        transitions=catalog_offer_flow("services.provider"),
    ),
    ("services", "service_contracts"): ResourceRule(
        ...
        transitions={
            "accept": Transition(
                frozenset({"draft"}), "held", event="services.contract.created"
            ),
            "complete": Transition(
                frozenset({"held", "in_progress"}),
                "completed",
                event="services.contract.completed",
            ),
            "dispute": Transition(
                frozenset({"held", "in_progress"}),
                "disputed",
                event="services.contract.disputed",
            ),
        },
    ),
    ("mobility", "rides"): ResourceRule(
        ...
        transitions={
            "accept": Transition(
                frozenset({"requested"}), "accepted", event="mobility.ride.accepted"
            ),
            "complete": Transition(
                frozenset({"accepted", "in_progress"}),
                "completed",
                event="mobility.ride.completed",
            ),
            "cancel": Transition(
                frozenset({"requested", "accepted"}),
                "cancelled",
                event="mobility.ride.cancelled",
            ),
        },
    ),
    ...
    ("mobility", "tickets"): ResourceRule(
        ...
        transitions={
            "use": Transition(
                frozenset({"active"}), "used", event="mobility.ticket.used"
            )
        },
    ),
    ...
    ("jobs", "applications"): ResourceRule(
        ...
        transitions={
            "withdraw": Transition(
                ...
                "withdrawn",
                event="jobs.application.withdrawn",
            ),
            "review": Transition(
                frozenset({"submitted"}),
                "under_review",
            ...
            "reject": Transition(
                frozenset({"submitted", "under_review", "shortlisted"}),
                "rejected",
                RECRUITER_ROLES,
                True,
    ...
    ("erp", "accounts"): ResourceRule(
        ("account_code", "name"),
        initial_status="active",
        sensitive=True,
    ),
    ("erp", "cost_centers"): ResourceRule(
        ("cost_center_code", "name"),
        initial_status="active",
        sensitive=True,
    ),
    ("erp", "payables"): ResourceRule(
        ("supplier_name", "due_at", "amount_brl", "cost_center_id"),
        initial_status="open",
        sensitive=True,
        monetary_fields=("amount_brl",),
        transitions={
            "approve_payment": Transition(
                frozenset({"open"}),
                "approved",
                APPROVER_ROLES,
                True,
            ...
            "settle": Transition(
                frozenset({"approved"}),
                "paid",
                event="erp.payable.paid",
            ),
        },
    ),
    ("erp", "receivables"): ResourceRule(
        ("customer_name", "due_at", "amount_brl", "account_id"),
    ...
    ("erp", "fiscal_documents"): ResourceRule(
        ("document_type", "amount_brl"),
        monetary_fields=("amount_brl"),
    ...
    ("hr", "occupational_records"): ResourceRule(
        ("employee_id",),
        sensitive=True,
        transitions=review_flow("hr.occupational_record"),
    ),
    ("health", "patients"): ResourceRule(("health_identifier",), sensitive=True),
    ("health", "appointments"): ResourceRule(
        ("scheduled_at",),
        sensitive=True,
        transitions=catalog_offer_flow("health.appointment"),
    ),
    ("health", "medical_records"): ResourceRule(
        ("patient_id",),
        sensitive=True,
        transitions=review_flow("health.medical_record"),
    ),
    ("health", "prescriptions"): ResourceRule(
        ("patient_id",), sensitive=True, transitions=review_flow("health.prescription")
    ),
    ...
    ("ai_core", "ai_memories"): ResourceRule(
        ("memory_key", "summary"),
        initial_status="draft",
        sensitive=True,
        transitions={
            "index": Transition(
                frozenset({"draft", "updated"}),
                "indexed",
                event="ai.memory.indexed",
            ),
            "update": Transition(
                frozenset({"draft", "indexed", "updated"}),
                "updated",
                event="ai.memory.updated",
            ),
        },
    ),
    ("ai_core", "moderation_decisions"): ResourceRule(
        ("module", "risk_score"),
        sensitive=True,
        transitions=review_flow("ai.moderation"),
    ),
    ("ai_core", "model_runs"): ResourceRule(
        (
            "provider_adapter",
            "provider_name",
            "model_name",
        ...
        transitions={
            "complete": Transition(
                frozenset({"requested"}),
                "completed",
                event="ai.model_run.completed",
            ),
            "fail": Transition(
                frozenset({"requested"}),
                "failed",
                event="ai.model_run.failed",
            ),
            "approve_cost": Transition(
                frozenset({"completed"}),
                "cost_approved",
                APPROVER_ROLES,
                True,
    ...
    ("api_hub", "api_clients"): ResourceRule(
        ("client_name", "scopes"), sensitive=True, transitions=review_flow("api.client")
    ),
    ("api_hub", "api_keys"): ResourceRule(
        ("key_name", "key_hash", "key_hint", "scopes"),
        sensitive=True,
        transitions=review_flow("api.key"),
    ),
    ("api_hub", "webhooks"): ResourceRule(
        ("target_url", "event_patterns", "signing_secret_reference"),
        sensitive=True,
        transitions=review_flow("api.webhook"),
    ),
    ("api_hub", "integration_runs"): ResourceRule(
        ("integration_type", "provider_name"),
        sensitive=True,
        transitions=review_flow("api.integration_run"),
    ),
}
...
def default_rule(module: str, resource_type: str) -> ResourceRule:
    return ResourceRule(
        transitions=lifecycle_flow(f"{module}.{resource_type.rstrip('s')}")
    )
...
def rule_for(module: str, resource_type: str) -> ResourceRule:
    if resource_type not in MODULE_ENTITIES.get(module, ()):
        raise HTTPException(
            status_code=404,
            detail=f"Recurso nao contratado para {module}: {resource_type}.",
        )
    return RULE_OVERRIDES.get(
        (module, resource_type), default_rule(module, resource_type)
    )


def check_payload(rule: ResourceRule, payload: dict[str, Any]) -> None:
    missing = [
        field for field in rule.required_fields if payload.get(field) in (None, "")
    ]
    if missing:
        raise HTTPException(
            status_code=422,
    ...
    if rule.protected_content:
        ...
        if any(pattern.search(material) for pattern in OFF_PLATFORM_PATTERNS):
            raise HTTPException(
                status_code=422, detail="Conteudo bloqueado pela politica anti-burla."
            )
    ...
    if "cpf_document" in payload and not CPF_DOCUMENT.fullmatch(
        str(payload["cpf_document"])
    ):
        raise HTTPException(
            status_code=422, detail="Documento CPF/internacional invalido."
        )
    ...
    if payload.get("company_status") and payload["company_status"] not in {
        "approved",
        "active",
    }:
        raise HTTPException(status_code=422, detail="Operacao exige empresa aprovada.")
    if payload.get("recruiter_visibility") and payload["recruiter_visibility"] not in {
        "private",
        "business_recruiters",
    }:
        raise HTTPException(
            status_code=422, detail="Visibilidade de curriculo invalida."
        )
    if (
        payload.get("role")
        and str(payload["role"]).casefold() not in BUSINESS_MEMBERSHIP_ROLES
    ):
        raise HTTPException(
            status_code=422, detail="Papel Business invalido para membership."
        )
    ...
    if storage_provider and storage_provider not in {
        "private_vault",
        "gcs_kms",
        "s3_kms",
    }:
        raise HTTPException(
            status_code=422, detail="storage_provider privado invalido."
        )
    if storage_bucket and "public" in storage_bucket.casefold():
        raise HTTPException(
            status_code=422, detail="storage_bucket publico nao permitido."
        )
...
def event_for_create(module: str, resource_type: str) -> str:
    explicit = {
        ...
        ("erp", "accounts"): "erp.account.created",
        ("erp", "cost_centers"): "erp.cost_center.created",
        ...
        ("ai_core", "ai_memories"): "ai.memory.created",
        ("ai_core", "moderation_decisions"): "ai.moderation.created",
        ("ai_core", "model_runs"): "ai.model_run.requested",
    ...
    return explicit.get(
        (module, resource_type), f"{module}.{resource_type.rstrip('s')}.created"
    )
