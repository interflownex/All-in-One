...
class ServicesPostgresStore:
    ...
    def create(
        ...
        try:
            with self.transaction() as connection:
                ...
                if item is None:
                    raise RuntimeError(
                        "PostgreSQL nao retornou recurso Services criado."
                    )
                self._audit(
                    connection,
                    actor,
        ...
        if resource_type == "service_contracts":
            return connection.execute(
                ...
                (
                    ...
                    payload.get("visit_price_brl"),
                    payload.get("contracted_price_brl"),
    ...
    def update(
        ...
        with self.transaction() as connection:
            ...
            if updated is None:
                raise RuntimeError(
                    "PostgreSQL nao retornou recurso Services atualizado."
                )
            self._audit(
                connection,
                actor,
    ...
    def metrics(self) -> tuple[int, int, int]:
        ...
        audits = self.connection.execute(
            "SELECT COUNT(*) AS count FROM audit.logs WHERE module = 'services'"
        ).fetchone()["count"]
