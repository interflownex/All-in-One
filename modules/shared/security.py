...
def actor_from_headers(
    ...
    x_valley_master_account: str = Header(
        default="false", alias="X-Valley-Master-Account"
    ),
    ...
    x_device_fingerprint: str | None = Header(
        default=None, alias="X-Device-Fingerprint"
    ),
    ...
    roles = frozenset(
        item.strip().casefold() for item in x_actor_roles.split(",") if item.strip()
    )
    scopes = frozenset(
        item.strip().casefold() for item in x_actor_scopes.split(",") if item.strip()
    )
    actor = Actor(
        x_actor_user_id,
        roles,
    ...
    channel = (
        x_client_channel.casefold()
        if x_client_channel.casefold() in allowed_channels
        else "api"
    )
    set_audit_context(
        AuditContext(
            ...
            ip_address=x_forwarded_for.split(",", maxsplit=1)[0].strip()[:64]
            if x_forwarded_for
            else None,
            user_agent=user_agent[:500] if user_agent else None,
            origin=x_client_origin.casefold()[:100],
            channel=channel,
...
def _verify_gateway_signature(
    actor: Actor, timestamp: str | None, signature: str | None
) -> None:
    ...
    if not timestamp or not signature:
        raise HTTPException(
            status_code=401, detail="Assinatura do gateway obrigatoria."
        )
    try:
        issued_at = int(timestamp)
    except ValueError:
        raise HTTPException(
            status_code=401, detail="Timestamp do gateway invalido."
        ) from None
...
def demand_role(actor: Actor, accepted: frozenset[str], action: str) -> None:
    if accepted and not actor.roles.intersection(accepted):
        raise HTTPException(
            status_code=403, detail=f"Perfil sem permissao para {action}."
        )
...
def demand_active_business_recruiter(
    ...
    if actor.business_id is None or actor.business_status != "active":
        raise HTTPException(
            status_code=403,
            detail="Acesso exclusivo a empresa ativa no All-in-One Business.",
        )
...
def demand_active_business(
    actor: Actor, action: str = "operar no Valley Business"
) -> None:
    if actor.business_id is None or actor.business_status != "active":
        raise HTTPException(
            status_code=403,
    ...
    demand_role(
        actor,
        frozenset({"owner", "administrator", "merchant", "store_manager", "auditor"}),
        action,
    )
...
def demand_valley_master_stock(actor: Actor) -> None:
    if (
        actor.valley_master_account
        or "valley_master" in actor.roles
        or "stock:global:import" in actor.scopes
    ):
        return
    raise HTTPException(
        status_code=403,
        detail="Modulo Stock global exclusivo da conta mestre corporativa Valley.",
    )
...
def enforce_essential_plan(actor: Actor, payload: dict[str, object]) -> None:
    ...
    if not actor.business_cnpj:
        raise HTTPException(
            status_code=403,
            detail="Plano Essencial exige CNPJ autenticado no contexto Business.",
        )
    blocked = (
        payload.get("external_integrations")
        or payload.get("integration_provider")
        or payload.get("external_provider")
    )
    if blocked:
        raise HTTPException(
            status_code=403,
            detail="Integracoes externas bloqueadas no Plano Essencial.",
        )
    ...
    if payload_cnpj and _digits_only(str(payload_cnpj)) != actor.business_cnpj:
        raise HTTPException(
            status_code=403,
            detail="Plano Essencial permite apenas o CNPJ autenticado da propria unidade.",
        )
    root_cnpj = payload.get("root_cnpj")
    if root_cnpj and _digits_only(str(root_cnpj)) != actor.business_cnpj:
        raise HTTPException(
            status_code=403,
            detail="Plano Essencial nao permite matriz/filiais no mesmo cadastro.",
        )
