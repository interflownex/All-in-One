...
from .dynamic_forms import DynamicFormValidationError, assert_transition, validate_blueprint, validate_submission_values
...
class DynamicFormsPostgresStore:
    ...
    def create_definition(
        ...
        *,
        ...
        try:
            with self.transaction() as connection:
                definition = connection.execute(
                    """INSERT INTO forms.form_definitions
                       ...
                       RETURNING *""",
                    (definition_id, tenant_id, company_id, module_id, business_context, name, description, actor_user_id, actor_user_id),
                ).fetchone()
            ...
            raise DynamicFormValidationError("Formulario ou chave de idempotencia ja existente no tenant.") from exc

    def list_definitions(self, tenant_id: str, *, status: str | None = None) -> list[dict[str, Any]]:
    ...
    def get_blueprint(self, tenant_id: str, version_id: str, *, connection: Connection | None = None, lock: bool = False) -> dict[str, Any]:
        ...
        version = executor.execute(
            """SELECT v.*, d.tenant_id, d.company_id, d.module_id, d.business_context, d.name AS form_name
               ...
               WHERE v.id = %s AND d.tenant_id = %s AND d.deleted_at IS NULL""" + lock_sql,
        ...
        for key, table in collections.items():
            order = "display_order, id" if key in {"blocks", "fields", "calculations"} else "created_at, id"
            blueprint[key] = [
                dict(row)
    ...
    def replace_blueprint(self, tenant_id: str, version_id: str, blueprint: dict[str, Any], actor_user_id: str) -> dict[str, Any]:
        with self.transaction() as connection:
            current = self.get_blueprint(tenant_id, version_id, connection=connection, lock=True)
            if current["version"]["status"] not in {"draft", "editing", "changes_requested", "rejected"}:
                raise DynamicFormValidationError("Somente versao editavel pode receber alteracoes.")
            catalog_ids = sorted({str(item.get("field_catalog_id")) for item in blueprint.get("fields") or []})
            binding_ids = sorted({str(item.get("field_binding_id")) for item in blueprint.get("fields") or []})
            ...
            self._insert_blueprint(connection, version_id, validated["blueprint"], actor_user_id)
            connection.execute(
    ...
    def request_homologation(
        ...
        with self.transaction() as connection:
            ...
            if not current["blueprint"]["fields"]:
                raise DynamicFormValidationError("Formulario vazio nao pode ser homologado.")
    ...
    def review_homologation(
        ...
        target_status = {"approved": "approved", "changes_requested": "changes_requested", "rejected": "rejected"}.get(result)
        ...
        with self.transaction() as connection:
            ...
            if str(homologation["requester_id"]) == actor_user_id:
                raise DynamicFormValidationError("Segregacao de funcoes: solicitante nao pode homologar a propria versao.")
            ...
            updated = connection.execute(
                ...
                (result, actor_user_id, notes, Jsonb(problems), Jsonb(corrections), Jsonb(evidence), target_status, homologation_id),
            ).fetchone()
            if target_status == "approved":
                ...
                connection.execute(
                    ...
                    (actor_user_id, notes or "Homologacao rejeitada.", homologation["form_version_id"]),
                )
            else:
                connection.execute(
            ...
            self._billing_event(
                ...
                event_type="homologation_approved" if target_status == "approved" else "change_requested",
                actor_user_id=actor_user_id,
                idempotency_key=idempotency_key,
            )
            return dict(updated)
    ...
    def publish_version(
        ...
        with self.transaction() as connection:
            ...
            if approved is None or not version.get("checksum"):
                raise DynamicFormValidationError("Publicacao exige homologacao aprovada e checksum.")
            publication = connection.execute(
                ...
                (version_id, environment, actor_user_id, Jsonb(rollout_policy), Jsonb(tenant_scope), Jsonb(channels), version["checksum"]),
            ).fetchone()
            connection.execute(
            ...
            event_type = "initial_publication" if int(previous["total"]) == 1 else "new_version_published"
            self._billing_event(
                connection,
                tenant_id=tenant_id,
        ...
        with self.transaction() as connection:
            ...
            submission = connection.execute(
                """INSERT INTO forms.form_submissions
                   ...
                   RETURNING *""",
                (submission_id, definition_id, version_id, actor_user_id, tenant_id, Jsonb(context), definition["business_context"], source, correlation_id, idempotency_key, Jsonb({"valid": True, "field_count": len(normalized)})),
            ).fetchone()
            for value in normalized:
                connection.execute(
                    ...
                    (submission_id, value["field_catalog_id"], value["data_type"], Jsonb(value["normalized_value"]), value["display_value"], value["unit"], Jsonb({"valid": True}), value["sensitivity"], "field_level" if value["sensitivity"] in {"personal", "sensitive", "restricted"} else "platform_managed"),
                )
            event_item = {
                "id": submission_id,
                ...
                "entity_id": tenant_id,
                "idempotency_key": idempotency_key,
                "payload": {"form_definition_id": definition_id, "form_version_id": version_id, "status": "completed", "field_count": len(normalized), "source": source},
            }
            ...
            connection.execute(
                """INSERT INTO audit.domain_events
                   ...
                   VALUES (%s, %s, %s, %s, 'forms.submission.completed', 'form_submissions', %s,
                           %s, %s, %s, %s)""",
                (event_id, actor_user_id, actor_user_id, definition.get("company_id"), submission_id, correlation_id, EVENT_SCHEMA_VERSION, Jsonb(envelope), actor_user_id),
            )
            connection.execute(
    ...
    def _delete_blueprint(connection: Connection, version_id: str) -> None:
        for table in ("form_calculations", "form_validations", "form_fields", "form_blocks", "form_visibility_rules"):
            connection.execute(f"DELETE FROM forms.{table} WHERE form_version_id = %s", (version_id,))
    ...
    def _ordered_blocks(blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        while pending:
            ready = [item for item in pending.values() if not item.get("parent_block_id") or str(item["parent_block_id"]) not in pending]
            if not ready:
                raise DynamicFormValidationError("Ciclo de blocos impediu persistencia.")
            for item in sorted(ready, key=lambda row: (int(row.get("display_order", 0)), str(row["id"]))):
                ordered.append(item)
                pending.pop(str(item["id"]))
        return ordered

    def _insert_blueprint(self, connection: Connection, version_id: str, blueprint: dict[str, Any], actor_user_id: str) -> None:
        for rule in blueprint["visibility_rules"]:
            connection.execute(
                ...
                (rule["id"], version_id, rule["target_type"], rule["target_id"], Jsonb(rule["condition"]), rule["operator"], Jsonb(rule.get("comparison_value")), rule["result"], rule.get("priority", 0), rule.get("combination", "and"), rule.get("status", "active"), actor_user_id),
            )
        for block in self._ordered_blocks(blueprint["blocks"]):
            connection.execute(
                ...
                (block["id"], version_id, block["block_type"], block.get("parent_block_id"), block.get("display_order", 0), block["title"], block.get("description"), block.get("width", 12), block.get("collapsible", False), block.get("visibility_rule_id"), block.get("repeatable", False), block.get("allowed_style", "default"), actor_user_id),
            )
            ...
            connection.execute(
                """INSERT INTO forms.form_fields
                   ...
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                           %s, %s, %s, %s, %s, %s, %s, %s)""",
                (field["id"], version_id, field["block_id"], field["field_catalog_id"], field["field_binding_id"], field["component"], field["label"], field.get("help_text"), field.get("placeholder"), field.get("required", False), field.get("read_only", False), field.get("hidden", False), field.get("display_order", 0), field.get("width", 12), field.get("mask"), field.get("format"), Jsonb(field.get("default_value")), field.get("value_source", "user"), field.get("unit"), Jsonb(field.get("permissions") or {}), field.get("visibility_rule_id"), Jsonb(field.get("validation_ids") or []), Jsonb(field.get("audit_policy") or {}), actor_user_id),
            )
        for validation in blueprint["validations"]:
            connection.execute(
                """INSERT INTO forms.form_validations
                   ...
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                (validation["id"], version_id, validation.get("field_id"), validation["validation_type"], Jsonb(validation.get("parameters") or {}), validation.get("message_pt_br") or "Valor invalido.", validation.get("severity", "error"), Jsonb(validation.get("condition")), validation.get("run_frontend", True), validation.get("run_backend", True), validation.get("status", "active"), validation.get("version", 1), actor_user_id),
            )
        for calculation in blueprint["calculations"]:
            connection.execute(
                ...
                (calculation["id"], version_id, calculation.get("name") or str(calculation["id"]), calculation["result_field_id"], Jsonb(calculation["operand_field_ids"]), calculation["operation"], Jsonb(calculation["safe_expression"]), calculation.get("display_order", 0), calculation.get("precision"), calculation.get("rounding"), calculation.get("trigger_mode", "on_change"), Jsonb(calculation.get("condition")), calculation.get("unit"), calculation.get("null_handling", "error"), calculation.get("division_by_zero_handling", "error"), calculation.get("visibility", "visible"), Jsonb(calculation.get("validation") or {}), calculation.get("status", "active"), calculation.get("version", 1), actor_user_id),
            )
    ...
    def _billing_event(
        ...
        *,
        ...
        connection.execute(
            ...
            (tenant_id, definition_id, version_id, event_type, actor_user_id, idempotency_key, Jsonb({"source": "dynamic_forms_api"})),
        )
