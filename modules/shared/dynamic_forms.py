...
FORBIDDEN_TEXT = re.compile(
    r"(?:\b(?:select|insert|update|delete|drop|alter|create|grant|revoke)\b|<script|javascript:|;|--|/\*)",
    re.I,
)
MAX_BLOCKS = 50
MAX_FIELDS = 200
...
def assert_transition(current: str, target: str) -> None:
    if target not in VERSION_TRANSITIONS.get(current, frozenset()):
        raise DynamicFormValidationError(
            f"Transicao de versao nao permitida: {current} -> {target}."
        )
    ...
    if isinstance(value, dict):
        for raw_key, child in value.items():
            ...
            if key in FORBIDDEN_KEYS:
                raise DynamicFormValidationError(
                    f"Propriedade proibida em {path}: {raw_key}."
                )
...
def _assert_acyclic(
    edges: dict[str, set[str]], nodes: Iterable[str], label: str
) -> None:
...
def validate_blueprint(
    ...
    *,
    ...
    if not blocks or not fields:
        raise DynamicFormValidationError(
            "Blueprint precisa conter ao menos um bloco e um campo."
        )
    for rows, limit, label in (
        ...
        if len(rows) > limit:
            raise DynamicFormValidationError(
                f"Limite de {label} excedido: maximo {limit}."
            )
    ...
    visibility_ids = (
        _unique_ids(visibility_rules, "visibility_rules") if visibility_rules else set()
    )
    ...
    for block in blocks:
        ...
        if block.get("block_type") not in {"section", "group", "tab", "column"}:
            raise DynamicFormValidationError(
                f"Tipo de bloco nao permitido: {block.get('block_type')}."
            )
        if not 1 <= int(block.get("width", 12)) <= 12:
            raise DynamicFormValidationError(
                "Largura de bloco deve estar entre 1 e 12."
            )
        visibility = block.get("visibility_rule_id")
        if visibility and str(visibility) not in visibility_ids:
            raise DynamicFormValidationError(
                f"Regra de visibilidade inexistente: {visibility}."
            )
    ...
    for validation in validations:
        ...
        if field_id and str(field_id) not in field_ids:
            raise DynamicFormValidationError(
                f"Validacao aponta para campo inexistente: {field_id}."
            )
        if field_id:
            validation_types_by_field[str(field_id)].add(
                str(validation.get("validation_type"))
            )
        if validation.get("severity") in {"error", "blocking"} and not validation.get(
            "run_backend", True
        ):
            raise DynamicFormValidationError(
                "Validacao bloqueante precisa executar no backend."
            )
    ...
    for field in fields:
        ...
        if block_id not in block_ids:
            raise DynamicFormValidationError(
                f"Campo {field_id} aponta para bloco inexistente."
            )
        if catalog_id not in catalog or catalog[catalog_id].get("status") != "active":
            raise DynamicFormValidationError(
                f"Campo de catalogo nao autorizado: {catalog_id}."
            )
        if binding_id not in bindings or bindings[binding_id].get("status") != "active":
            raise DynamicFormValidationError(
                f"Binding logico nao autorizado: {binding_id}."
            )
        if str(bindings[binding_id].get("field_catalog_id")) != catalog_id:
            raise DynamicFormValidationError(
                "Binding nao pertence ao campo de catalogo informado."
            )
        component = str(field.get("component") or "")
        allowed = (
            set(catalog[catalog_id].get("allowed_components") or [])
            & ALLOWED_COMPONENTS
        )
        if component not in allowed:
            raise DynamicFormValidationError(
                f"Componente {component} nao permitido para {catalog_id}."
            )
        ...
        if not mandatory.issubset(declared):
            ...
            raise DynamicFormValidationError(
                f"Validacoes estruturais ausentes em {field_id}: {missing}."
            )
        if not 1 <= int(field.get("width", 12)) <= 12:
            raise DynamicFormValidationError(
                "Largura de campo deve estar entre 1 e 12."
            )
        ...
        referenced_validations = {
            str(item) for item in field.get("validation_ids") or []
        }
        if not referenced_validations.issubset(validation_ids):
            raise DynamicFormValidationError(
                f"Campo {field_id} referencia validacao inexistente."
            )
    ...
    for calculation in calculations:
        ...
        if result not in field_ids or not operands or not operands.issubset(field_ids):
            raise DynamicFormValidationError(
                f"Campos invalidos no calculo {calculation_id}."
            )
        ...
        result_catalog = catalog[
            str(
                next(
                    ...
                    for field in fields
                    if str(field["id"]) == result
                )
            )
        ]
        if operation not in set(result_catalog.get("allowed_calculations") or []):
            raise DynamicFormValidationError(
                f"Calculo {operation} nao autorizado para o campo de resultado."
            )
    ...
    for calculation_id, operands in calculation_edges.items():
        dependency_edges[calculation_id] = {
            calculated_by_result[item]
            for item in operands
            if item in calculated_by_result
        }
    _assert_acyclic(
        dependency_edges,
        [str(item["id"]) for item in calculations],
        "dependencias de calculo",
    )
...
def validate_submission_values(
    *,
    ...
    allowed = {
        str(field["field_catalog_id"]): field
        for field in fields
    ...
    if unknown:
        raise DynamicFormValidationError(
            f"Campos nao autorizados na submissao: {', '.join(sorted(unknown))}."
        )
    normalized: list[dict[str, Any]] = []
    for catalog_id, field in allowed.items():
        present = catalog_id in values
        if field.get("required") and (
            not present or values[catalog_id] in (None, "", [])
        ):
            raise DynamicFormValidationError(
                f"Campo obrigatorio ausente: {catalog_id}."
            )
        if not present:
            continue
        if field.get("read_only"):
            raise DynamicFormValidationError(
                f"Campo somente leitura nao aceita entrada: {catalog_id}."
            )
        definition = catalog.get(catalog_id)
        if definition is None or definition.get("status") != "active":
            raise DynamicFormValidationError(
                f"Campo de catalogo indisponivel: {catalog_id}."
            )
        ...
        try:
            ...
            raise DynamicFormValidationError(
                f"Tipo invalido para {catalog_id}: esperado {data_type}."
            ) from None
        normalized.append(
            {
