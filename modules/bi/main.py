...
def commercial_insights_series(
    ...
    series: dict[str, dict[str, Any]] = defaultdict(lambda: {
        "orders_total": 0,
        "orders_paid": 0,
    ...
    for order in orders:
        ...
        if order["status"] in {"paid", "accepted", "in_progress", "delivered", "completed"}:
    ...
    for date_str in sorted(series.keys()):
        ...
        avg_rating = round(data["reviews_sum"] / data["reviews_count"], 2) if data["reviews_count"] > 0 else None
        conversion = round((data["orders_paid"] / data["orders_total"]) * 100, 2) if data["orders_total"] > 0 else 0.0
