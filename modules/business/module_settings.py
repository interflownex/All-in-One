...
ModuleState = Literal['mandatory', 'active', 'recommended', 'optional', 'hidden', 'disabled', 'blocked_by_plan']
...
def _dependencies(module_slug: str) -> list[str]:
    ...
    return [dep for dep in ['identity', 'business', 'permissions'] if dep != module_slug]


def _explanation(module_slug: str, state: ModuleState, business_kind: BusinessKind) -> str:
    ...
    if state == 'disabled':
        return f'{module_name} foi desativado manualmente e nao deve executar operacoes.'
    return f'{module_name} depende de plano, permissao, configuracao ou revisao administrativa.'


def recommend_business_modules(input_data: BusinessClassificationInput) -> list[ModuleRecommendation]:
    ...
    active = _unique([*preset['active'], *[module for module in dynamic_recommended if module not in mandatory]])
    recommended = [module for module in _unique(preset['recommended']) if module not in mandatory and module not in active]
    hidden = [module for module in _unique(preset['hidden']) if module not in mandatory and module not in active and module not in recommended]
...
def _classification(company_id: UUID, input_data: BusinessClassificationInput) -> BusinessClassificationRecord:
    operational_tags = [
        tag
        for tag, enabled in {
...
def _audit(company_id: UUID, action: str, payload: dict[str, Any]) -> None:
    _AUDIT.setdefault(company_id, []).insert(
        0,
        {'id': str(uuid4()), 'action': action, 'payload': payload, 'created_at': _now()},
    )


@router.post('/recommendations', response_model=list[ModuleRecommendation])
def preview_recommendations(body: BusinessClassificationInput) -> list[ModuleRecommendation]:
    return recommend_business_modules(body)


@router.post('/companies/{company_id}/classification', response_model=BusinessClassificationRecord)
def classify_company(company_id: UUID, body: BusinessClassificationInput) -> BusinessClassificationRecord:
    record = _classification(company_id, body)
    _CLASSIFICATIONS[company_id] = record
    _audit(company_id, 'business.classification.generated', record.model_dump(mode='json'))
    return record


@router.post('/companies/{company_id}/apply-recommendations', response_model=ModuleSettingsResponse)
def apply_recommendations(company_id: UUID, body: ApplyRecommendationsPayload) -> ModuleSettingsResponse:
    ...
    _audit(company_id, 'business.module.recommendations_applied', {'modules': [item.model_dump(mode='json') for item in settings.values()]})
    return ModuleSettingsResponse(company_id=company_id, classification=classification, modules=list(settings.values()), audit=_AUDIT.get(company_id, []))
...
@router.patch('/companies/{company_id}/modules/{module_slug}', response_model=CompanyModuleSetting)
def patch_company_module(company_id: UUID, module_slug: str, body: ModulePatch) -> CompanyModuleSetting:
    settings = _SETTINGS.setdefault(company_id, {})
    current = settings.get(module_slug)
    if current and not current.can_disable and body.state not in {'mandatory', 'active'}:
        raise HTTPException(status_code=409, detail='Modulo obrigatorio nao pode ser ocultado, desativado ou bloqueado manualmente.')
    if current is None:
    ...
    _audit(company_id, 'business.module.configuration_updated', updated.model_dump(mode='json'))
    return updated


@router.get('/companies/{company_id}/modules/{module_slug}/change-impact')
def get_change_impact(company_id: UUID, module_slug: str, next_state: ModuleState = 'hidden') -> dict[str, Any]:
    current = _SETTINGS.get(company_id, {}).get(module_slug)
    if current and not current.can_disable and next_state not in {'mandatory', 'active'}:
        return {
            'allowed': False,
