...
def test_compose_postgres_healthcheck_allows_cold_initdb() -> None:
    compose = yaml.safe_load(
        (ROOT / "infra/docker/docker-compose.yml").read_text(encoding="utf-8")
    )
...
def test_compose_health_workflow_runs_python_gate_instead_of_sleep_only() -> None:
    workflow = (ROOT / ".github/workflows/compose-health.yml").read_text(
        encoding="utf-8"
    )
...
def test_compose_health_gate_reports_pending_services(monkeypatch) -> None:
    ...
    def fake_is_healthy(
        probe: validate_compose_health.ServiceProbe, timeout_seconds: float
    ) -> bool:
    ...
    pending = validate_compose_health.wait_for_health(
        timeout_seconds=0, probe_timeout_seconds=0.01
    )
...
def test_compose_health_gate_times_out_docker_commands(monkeypatch) -> None:
    ...
    validate_compose_health.run_checked(
        ["docker", "compose", "config", "--quiet"], timeout_seconds=123
    )

    assert calls == [(["docker", "compose", "config", "--quiet"], 123)]


def test_compose_health_gate_accepts_env_file_and_project_name(
...
def test_compose_health_gate_can_fail_fast_on_occupied_ports(
