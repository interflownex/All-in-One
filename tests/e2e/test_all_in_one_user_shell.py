...
def test_all_in_one_user_shell_submits_live_job_application(
    ...
    expect(jobs_status).to_contain_text("vaga publicada encontrada", timeout=LIVE_ACTION_TIMEOUT)

    page.get_by_placeholder("Buscar em Job Postings...").fill("Jornada")
    page.get_by_role("button", name="Pesquisar").click()
    expect(jobs_status).to_contain_text("Busca aplicada: Jornada", timeout=LIVE_ACTION_TIMEOUT)
    expect(page.get_by_text("Vaga Playwright")).to_be_visible(timeout=LIVE_ACTION_TIMEOUT)
    ...
    expect(jobs_status).to_contain_text("Notificacao Jobs: candidatura enviada com sucesso.", timeout=LIVE_ACTION_TIMEOUT)
