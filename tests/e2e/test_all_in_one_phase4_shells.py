...
def test_phase4_shells_load_api_hub_data(
    ...
    expect(
        page.get_by_text("registro(s) retornado(s) pelo API Hub").first
    ).to_be_visible()
...
def test_phase4_shells_load_live_api_hub_fixtures(
    ...
    expect(
        page.get_by_text("1 registro(s) retornado(s) pelo API Hub").first
    ).to_be_visible()


def test_services_shell_completes_live_contract_journey(
    ...
    expect(post_panel.get_by_text("Escrow operacional vinculado")).to_be_visible(
        timeout=15000
    )
    expect(post_panel.get_by_text("Evidencia documental disponivel")).to_be_visible(
        timeout=15000
    )
    ...
    expect(action_panel.locator("dd", has_text="completed")).to_be_visible(
        timeout=15000
    )
    expect(action_panel.locator(".journey-feedback.completed")).to_contain_text(
        "Jornada concluida"
    )
    expect(post_panel.get_by_text("Pos-atendimento criado")).to_be_visible(
        timeout=15000
    )


def test_mobility_shell_completes_live_ride_and_ticket_journey(
    page: Page, request: pytest.FixtureRequest
) -> None:
    ...
    expect(
        post_panel.get_by_text("Ticket MOB-E2E com QR/NFC tokenizado")
    ...
    expect(post_panel.get_by_text("Comprovante pos-corrida criado")).to_be_visible(
        timeout=15000
    )


def test_riders_shell_approves_and_activates_live_profile(
    page: Page, request: pytest.FixtureRequest
) -> None:
    ...
    expect(
        operations_panel.get_by_text("Entrega disponivel para operacao")
    ).to_be_visible(timeout=15000)
    expect(
        operations_panel.get_by_text("Corrida disponivel para operacao")
    ).to_be_visible(timeout=15000)
    ...
    expect(operations_panel.get_by_text("Ganhos operacionais prontos")).to_be_visible(
        timeout=15000
    )


def test_health_shell_approves_and_completes_live_appointment(
    page: Page, request: pytest.FixtureRequest
) -> None:
    ...
    expect(governance_panel.get_by_text("Consentimento verificado")).to_be_visible(
        timeout=15000
    )
    expect(
        governance_panel.get_by_text("Prontuario protegido disponivel")
    ).to_be_visible(timeout=15000)
    ...
    expect(governance_panel.get_by_text("Retorno pos-consulta criado")).to_be_visible(
        timeout=15000
    )
