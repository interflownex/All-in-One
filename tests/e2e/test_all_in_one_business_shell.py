...
def test_all_in_one_business_shell_runs_live_api_hub_actions(
    ...
    expect(page.get_by_text("Empresa Business Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.get_by_label("Filtros operacionais Business")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    ...
    expect(page.get_by_text("Empresa aprovada no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.get_by_label("Auditoria operacional Business")).to_contain_text(
    ...
    expect(page.get_by_text("Vaga Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Publicar vaga").click()
    expect(page.get_by_text("Vaga publicada no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="published")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    ...
    expect(page.get_by_text("Acesso a curriculo registrado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.get_by_text("triagem para vaga publicada via Business shell")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/erp/fiscaldocuments", wait_until="domcontentloaded")
    expect(page.get_by_text("Relatorio de Giro Fiscal Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar registro ERP").click()
    expect(page.get_by_text("Registro ERP aprovado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/bi/dashboards", wait_until="domcontentloaded")
    expect(page.get_by_text("Dashboard Giro de Estoque Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar relatório BI").click()
    expect(page.get_by_text("Relatorio BI aprovado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/wms/warehouses", wait_until="domcontentloaded")
    expect(page.get_by_text("CD Regional Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar operação WMS").click()
    expect(page.get_by_text("Operacao WMS aprovada no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/tms/freights", wait_until="domcontentloaded")
    expect(page.get_by_text("Frete Regional Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar operação TMS").click()
    expect(page.get_by_text("Operacao TMS aprovada no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/crm/opportunities", wait_until="domcontentloaded")
    expect(page.get_by_text("Oportunidade B2B Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar oportunidade CRM").click()
    expect(page.get_by_text("Oportunidade CRM aprovada no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/bpm/processes", wait_until="domcontentloaded")
    expect(page.get_by_text("Fluxo BPM Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar fluxo BPM").click()
    expect(page.get_by_text("Fluxo BPM aprovado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    ...
    expect(page.get_by_text("Documento operacional aprovado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/hr/employees", wait_until="domcontentloaded")
    expect(page.get_by_text("Colaborador HR Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    page.get_by_role("button", name="Aprovar registro HR").click()
    expect(page.get_by_text("Registro HR aprovado no API Hub vivo.")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
    expect(page.locator(".badge", has_text="approved")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)

    page.goto(f"{all_in_one_business_live_server}/api_hub/apiclients", wait_until="domcontentloaded")
    expect(page.get_by_text("Cliente API Hub Playwright")).to_be_visible(timeout=LIVE_API_EXPECT_TIMEOUT)
