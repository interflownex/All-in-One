...
def test_docker_dx_defaults_are_secret_safe_and_keep_google_off_locally() -> None:
    ...
    assert all(
        "KEY" not in key and "SECRET" not in key and "TOKEN" not in key
        for key in defaults
    )
...
def test_docker_dx_capability_probe_does_not_hang(monkeypatch) -> None:
    monkeypatch.setattr(
        configure_docker_dx, "command_exists", lambda command: command == "docker"
    )
    ...
    assert (
        configure_docker_dx.docker_subcommand_exists(
            "compose", "version", timeout_seconds=1
        )
        is False
    )
...
def test_docker_dx_repairs_user_plugin_links_without_sudo(
    ...
    assert (
        user_plugins / "docker-compose"
    ).resolve() == system_plugins / "docker-compose"
    assert (
        user_plugins / "docker-buildx"
    ).resolve() == system_plugins / "docker-buildx"
    assert configure_docker_dx.ensure_user_cli_plugin_links(dry_run=False) == []
