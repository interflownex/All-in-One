...
def load_validator():
    spec = importlib.util.spec_from_file_location(
        "validate_data_audit_delivery", SCRIPT
    )
...
def test_contract_lists_every_complementary_format() -> None:
    ...
    assert (
        "artifacts/formulario_dinamico_modelo.json"
        in contract["required_complementary"]
    )
    assert "artifacts/coordenadas_stitch.json" in contract["required_complementary"]
    assert (
        "artifacts/modelo_unidades_tributacao.json"
        in contract["required_complementary"]
    )
    assert (
        "artifacts/politica_classificacao_campos.json"
        in contract["required_complementary"]
    )
    ...
    assert (
        "artifacts/catalogo_object_storage.json" in contract["required_complementary"]
    )
    assert (
        "artifacts/catalogo_browser_storage.json" in contract["required_complementary"]
    )
    assert (
        "artifacts/coordenada_projetos_stitch.json"
        in contract["required_complementary"]
    )
...
def test_generated_non_postgres_catalogs_have_field_evidence() -> None:
    ...
    mongodb = json.loads(
        (artifacts / "catalogo_mongodb.json").read_text(encoding="utf-8")
    )
    sqlite = json.loads(
        (artifacts / "catalogo_sqlite.json").read_text(encoding="utf-8")
    )
...
def test_stitch_coordinate_requires_exactly_three_resumable_product_projects() -> None:
    ...
    assert (
        data["continuation_policy"]["on_resource_exhausted"]
        == "registrar_checkpoint_e_retomar_na_proxima_execucao_agendada"
    )
    assert "prohibited_claim" in data["continuation_policy"]
    assert all(
        project["module_scope"] and project["surfaces"] for project in data["projects"]
    )
...
def test_generated_summary_is_rendered_markdown() -> None:
    summary = (ROOT / "docs" / "data-audit" / "00_RESUMO_EXECUTIVO.md").read_text(
        encoding="utf-8"
    )

    coverage = __import__("json").loads(
        (
            ROOT / "docs" / "data-audit" / "artifacts" / "checklist_cobertura.json"
...
def test_api_catalog_exposes_models_and_untyped_payloads() -> None:
    ...
    assert all(
        item["method"] and item["path"] and item["evidence"]
        for item in data["endpoints"]
    )
    assert any(not item["response_model"] for item in data["endpoints"])


def test_dynamic_form_model_exposes_partial_implementation_without_false_completion() -> (
    None
):
    model = (
        ROOT / "docs" / "data-audit" / "artifacts" / "formulario_dinamico_modelo.json"
    )
...
def test_ui_action_matrix_proves_the_generic_save_contract_alignment() -> None:
    ...
    assert all(
        "user_id" in item["request_contract"] and "payload" in item["request_contract"]
        for item in saves
    )
...
def test_permission_matrix_proves_horizontal_read_authorization_enforcement() -> None:
    matrix = (
        ROOT / "docs" / "data-audit" / "artifacts" / "matriz_enforcement_permissao.json"
    )
    data = __import__("json").loads(matrix.read_text(encoding="utf-8"))
    ...
    gaps = [
        ...
        if item["enforcement_status"] == "lacuna_autorizacao_horizontal"
    ]
    assert not gaps
    reads = [item for item in operations if item["operation"] == "read"]
    assert reads and all(
        item["ownership_enforcement"] == "owner_or_operator antes da exposição"
        for item in reads
    )
    assert any(
        item["operation"] == "approve" and item["role_enforcement"]
        for item in operations
    )
...
def test_requirement_test_matrix_does_not_promote_candidates_to_proof() -> None:
    ...
    catalog = __import__("json").loads(
        (artifacts / "catalogo_testes.json").read_text(encoding="utf-8")
    )
    matrix = __import__("json").loads(
        (artifacts / "matriz_requisito_teste.json").read_text(encoding="utf-8")
    )
    ...
    assert (
        catalog["counts"]["tests_with_assertions"] < catalog["counts"]["test_functions"]
    )
    assert catalog["counts"]["tests_with_http_calls"] > 0
    assert (
        matrix["counts"]["memo_requirements_traced"]
    ...
    assert all(
        candidate in test_ids
        for item in matrix["requirements"]
        for candidate in item["test_candidates"]
    )
    assert all(
        item["proof_status"].startswith("não comprovado")
        for item in matrix["requirements"]
    )
...
def test_units_and_tax_model_covers_precision_and_fiscal_governance() -> None:
    model = (
        ROOT / "docs" / "data-audit" / "artifacts" / "modelo_unidades_tributacao.json"
    )
    data = __import__("json").loads(model.read_text(encoding="utf-8"))
...
def test_every_logical_entity_has_a_frontend_surface_with_canonical_resource_name() -> (
    None
):
    catalog = ROOT / "docs" / "data-audit" / "artifacts" / "catalogo_logico.json"
    data = __import__("json").loads(catalog.read_text(encoding="utf-8"))
...
def test_audit_coverage_reports_every_structural_requirement() -> None:
    ...
    assert (
        data["counts"]["audit_requirements_covered"]
        == data["counts"]["audit_requirements"]
        == 35
    )
...
def test_every_gap_has_the_mandatory_execution_coordinate() -> None:
    ...
    assert all(
        gap["evidence"] and gap["affected_files"] and gap["dimensions"]
...
def test_coverage_links_only_dimension_specific_gaps_and_evidence() -> None:
    artifacts = ROOT / "docs" / "data-audit" / "artifacts"
    coverage = __import__("json").loads(
        (artifacts / "checklist_cobertura.json").read_text(encoding="utf-8")
    )
    report = __import__("json").loads(
        (artifacts / "relatorio_divergencias.json").read_text(encoding="utf-8")
    )
    ...
    for dimension, item in coverage["dimensoes"].items():
        ...
        assert item["metodo"]
        assert all(
            dimension in gaps_by_id[gap_id]["dimensions"] for gap_id in item["lacunas"]
        )
    assert coverage["dimensoes"]["bindings_frontend"]["lacunas"] == ["AUD-P1-002"]
    assert coverage["dimensoes"]["campos_sensiveis"]["lacunas"] == [
        "AUD-P0-001",
        "AUD-P1-007",
    ]
    assert "AUD-P1-007" in coverage["dimensoes"]["auditoria"]["lacunas"]
    assert not any(
        item["percentual"] == 100 and item["lacunas"]
        for item in coverage["dimensoes"].values()
    )
