...
def test_bundles_are_exactly_b0_to_b14() -> None:
    data = json.loads((ROOT / "config/compliance/bundles.v1.json").read_text(encoding="utf-8"))
...
def test_registry_has_frozen_baseline_and_valid_contract() -> None:
    ...
    assert validator.validate_registry(registry, bundles) == set()


def test_sql_parser_finds_create_and_alter_columns() -> None:
    validator = _load_validator()
    sql = """
...
def test_sensitive_literal_pattern_rejects_secrets() -> None:
    ...
    assert not validator.FORBIDDEN_LITERAL.search('client_secret_env = os.environ["CLIENT_SECRET"]')
