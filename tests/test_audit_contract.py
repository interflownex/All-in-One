...
def test_unified_audit_contract_covers_context_integrity_and_retention() -> None:
    ...
    record = build_audit_record(
        ...
        resource_id="company",
        before={"name": "Antes", "password": "segredo"},
...
def test_sensitive_read_contract_records_purpose_authorization_and_outputs() -> None:
    record = build_read_audit_record(
        module="health",
        actor_user_id="doctor",
        resource_type="medical_records",
        resource_id="record",
        purpose="Atendimento assistencial",
        context=AuditContext(authorization="health:records:read"),
        exported=True,
        printed=False,
        shared=False,
    )
...
def test_sqlite_store_persists_a_hash_chained_append_only_contract() -> None:
    ...
    first = store.audit_external(
        "actor", "read", "companies", "one", {"name": "Empresa"}
    )
    second = store.audit_external(
        "actor", "update", "companies", "one", {"name": "Nova"}
    )
...
def test_postgres_writer_persists_the_full_request_context() -> None:
    ...
    class Connection:
        def execute(self, query, params=None):
            ...
            return Result(
                ...
                else {"id": "audit-id", "row_hash": params[36]}
            )
    ...
    set_audit_context(
        AuditContext(
            ...
            channel="web",
            reason="Atendimento",
            authorization="health:records:read",
        )
    )
    evidence = insert_postgres_audit(
        ...
        resource_id="patient",
        before=None,
        after={"purpose": "Atendimento", "exported": True},
        user_id="patient",
        company_id=None,
    )
