...
def test_every_declared_typed_store_target_exists_in_the_physical_catalog() -> None:
    ...
    physical_tables = {f'{field["schema"]}.{field["table"]}' for field in dictionary["fields"]}
    missing: list[str] = []
...
def test_migration_026_is_reversible_for_every_created_relation() -> None:
    migration = (ROOT / "database/postgres/migrations/026_complete_typed_store_relations.sql").read_text(
        encoding="utf-8"
    )
    rollback = (ROOT / "database/postgres/rollbacks/026_complete_typed_store_relations.down.sql").read_text(
        encoding="utf-8"
    )
    created = {
        line.split()[5]
