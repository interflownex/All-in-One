...
def test_dynamic_forms_migration_materializes_every_governed_entity() -> None:
    ...
    for table in REQUIRED_TABLES:
        ...
        assert f"DROP TABLE IF EXISTS forms.{table}" in ROLLBACK.read_text(encoding="utf-8")
...
def test_dynamic_forms_migration_enforces_tenant_idempotency_and_review() -> None:
    ...
    assert "published_by IS NOT NULL AND approved_at IS NOT NULL AND checksum IS NOT NULL" in source
...
def test_dynamic_forms_migration_has_safe_calculation_and_component_allowlists() -> None:
    source = MIGRATION.read_text(encoding="utf-8")
...
def test_dynamic_forms_rollback_is_transactional_and_removes_schema_last() -> None:
    ...
    assert source.rfind("DROP SCHEMA IF EXISTS forms") > source.rfind("DROP TABLE IF EXISTS forms.field_catalog")
