...
def test_backup_restore_plan_covers_critical_assets_without_sensitive_payloads() -> None:
    plan = load_plan()
    ...
    for asset_name, asset in plan["assets"].items():
        ...
        assert asset["data_classification"] in {"critical", "sensitive", "operational"}, asset_name
        ...
        assert "KMS" in asset["encryption"] or "no_secret_values_in_git" in asset["encryption"], asset_name
...
def test_backup_restore_plan_has_quarterly_dr_exercise_and_success_criteria() -> None:
    ...
    assert "nenhum_segredo_ou_payload_sensivel_em_evidencia" in exercise["success_criteria"]
...
def test_backup_restore_plan_references_existing_local_validators() -> None:
    ...
    validations = {
        ...
        if item.endswith(".py") or item.endswith(".py --apply-migrations --repeat-migrations --write-checks")
    }

    assert "scripts/validate_postgres_real_dsn.py --apply-migrations --repeat-migrations --write-checks" in validations
