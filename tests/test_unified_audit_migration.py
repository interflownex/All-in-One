...
def test_audit_migration_materializes_context_read_integrity_and_retention() -> None:
    ...
    for column in (
        "tenant_id", "company_id", "actor_role", "session_id", "device_id", "origin", "channel",
        "changed_fields", "reason", "correlation_id", "causation_id", "occurred_at", "result",
        "error_detail", "authorization", "approval_id", "approved_by", "previous_hash", "row_hash",
        "exported", "printed", "shared", "retention_until",
    ):
...
def test_audit_migration_preserves_the_existing_append_only_trigger() -> None:
    prior = (ROOT / "database/postgres/migrations/005_audit_events_api_security.sql").read_text(encoding="utf-8")
