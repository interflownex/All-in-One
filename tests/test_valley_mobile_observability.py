...
def test_mobile_telemetry_is_opt_in_and_granular() -> None:
    contract = json.loads(
        (ROOT / "config" / "observability" / "valley_mobile_observability.json").read_text(encoding="utf-8")
    )
    manifest = (ANDROID / "AndroidManifest.xml").read_text(encoding="utf-8")

    assert contract["privacy"]["default_collection_enabled"] is False
    assert set(contract["privacy"]["granular_consent"]) == {"analytics", "crash_reports"}
...
def test_mobile_event_allowlist_excludes_sensitive_fields() -> None:
    ...
    event_fields = set(contract["events"]["api_request_completed"]["allowed_parameters"])
...
def test_mobile_availability_and_latency_have_slos_and_runbooks() -> None:
    ...
    assert all(item["runbook"] == "docs/OPERATIONS.md#valley-mobile" for item in slos.values())
