...
def test_git_sync_workflow_uses_python_gate() -> None:
    ...
    assert "python3 scripts/check_git_sync.py --branch main --remotes origin --allow-dirty" in workflow
    assert "shell: pwsh" not in workflow


def test_git_sync_python_gate_compiles() -> None:
    subprocess.run(["python3", "-m", "py_compile", "scripts/check_git_sync.py"], cwd=ROOT, check=True)
