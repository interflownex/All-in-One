...
**Data:** 2026-07-20  
**Branch:** `feat/brand-integrity-data-ui`  
**PR:** #15
...
|---|---|---|
| Blindagem da logomarca | Em execução | `BrandLogo` criado e regra documentada. Próximo passo: substituir todos os usos diretos do asset. |
| Verificação automática da marca | Em execução | `scripts/check_brand_integrity.py` criado. Próximo passo: conectar ao validador geral/CI. |
| Contrato de UI de dados | Em execução | `config/apps/data_ui_contract.json` define visão geral, relatórios, listas, formulários e estados. |
...
| Cockpit externo | Em execução | Publicação AppDeploy criada para acompanhamento público da entrega. |

## Próximas ações mandatórias

1. Substituir usos diretos de `<img src="/assets/brand/all-in-one-logo-light-official.png" />` pelo componente `BrandLogo`.
