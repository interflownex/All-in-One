...
| _Constraint_           | `UNIQUE`      | `(user_id, document_type, document_number_hash)` | Garante unicidade do documento por tipo para um usuário. |
...
_Este é um documento inicial. As demais tabelas deste arquivo e dos arquivos de migração subsequentes serão adicionadas progressivamente._
...
| _Constraint_ | `UNIQUE`      | `(user_id, company_id)`                         | Garante que um usuário só pode ser membro de uma empresa uma vez. |
| `...`        | `...`         | `...`                                           | (Campos de auditoria e outros omitidos por brevidade)             |
...
| _Constraint_    | `UNIQUE`        | `(id, user_id)`                             | Chave para referência segura em FKs.                  |
| `...`           | `...`           | `...`                                       | (Campos de auditoria e outros omitidos por brevidade) |
...
| _Constraint_      | `FOREIGN KEY`   | `(wallet_id, user_id) REFERENCES finance.wallets(id, user_id)` | Garante consistência do dono da carteira.    |
| `...`             | `...`           | `...`                                                          | (Campos de auditoria omitidos por brevidade) |
...
| _Constraint_     | `UNIQUE`        | `(store_id, sku)`                               | Garante unicidade do SKU por loja.             |
| `...`            | `...`           | `...`                                           | (Campos de auditoria omitidos por brevidade)   |
