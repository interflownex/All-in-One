...
| 3 | AUD-P1-002 | frontend e backend | Cada campo UI aponta para DTO, endpoint, validação, coluna e E2E aprovado. | decisão de domínio, ambiente homologado, credenciais de teste | implementacao_parcial |
| 4 | AUD-P1-003 | arquitetura de integração | Cada evento possui produtor, consumidor, schema, dados proibidos, idempotência, correlação, retenção e compatibilidade testados. | consumidores downstream, mensageria homologada | implementacao_parcial |
...
| 8 | AUD-P1-007 | segurança, plataforma e compliance | Os 35 requisitos de auditoria têm implementação ou decisão justificada por operação, com integridade, retenção e testes aprovados. | PostgreSQL de homologação, segurança, compliance, proprietários de domínio | implementacao_parcial |
