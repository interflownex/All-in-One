...
O inventário AST encontrou 482 funções de teste: 451 contêm `assert` e 191 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 457 funções, das quais 424 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
