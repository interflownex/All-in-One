...
O inventário AST encontrou 496 funções de teste: 462 contêm `assert` e 194 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 471 funções, das quais 438 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
