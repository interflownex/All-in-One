...
O inventário AST encontrou 535 funções de teste: 494 contêm `assert` e 204 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 510 funções, das quais 477 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
