...
O inventário AST encontrou 537 funções de teste: 496 contêm `assert` e 204 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 512 funções, das quais 479 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
