...
O inventário AST encontrou 573 funções de teste: 527 contêm `assert` e 212 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 502 funções, das quais 470 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
