...
O inventário AST encontrou 508 funções de teste: 474 contêm `assert` e 194 contêm chamadas HTTP reconhecidas. Os relatórios JUnit registram 483 funções, das quais 450 foram aprovadas; parametrizações são consolidadas por função. A presença de um teste ou candidato aprovado não comprova cobertura integral do requisito.
