...
Foram triados 529 campos potencialmente pessoais, sensíveis, financeiros, restritos ou pseudônimos vinculáveis. A política versionada registra categoria, padrão que motivou a triagem, criptografia, mascaramento e retenção. A classificação automática exige homologação pelo proprietário do domínio e revisão jurídica/privacidade quando aplicável.

Foram catalogadas 767 operações backend: 600 operações CRUD sobre 120 entidades e 187 transições. 125 possuem ao menos um arquivo de teste candidato localizado; isso não equivale a prova positiva/negativa completa por endpoint.
