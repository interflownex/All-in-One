...
| `permissions` | 72% | RBAC/ABAC modelado, store gerado, matriz versionada, consumidores sensiveis de dominio ligados a `sensitive_permissions_review` e testes HTTP negativos/positivos para endpoints Finance/Health | Homologar enforcement RBAC/ABAC em ambiente real e revisar ABAC fino por tenant/contexto | Executar revisao RBAC/ABAC com dados homologados e ampliar evidencias por modulo critico |
...
| `marketplace` | 73% | Catalogo, pedidos, store tipado, jornada pytest consumidor -> wallet -> escrow -> pedido -> pagamento sandbox, suporte/disputa e reviews com moderacao basica auditavel | Checkout completo, pagamento real e fulfillment com provider homologado | Expandir checkout/fulfillment com provedor homologado e publicar reviews moderadas em ambiente vivo |
