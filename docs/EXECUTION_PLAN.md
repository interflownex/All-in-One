...
| Apps/frontend | 75% | 9 apps catalogados, catalogo Valley backend regionalizado, plano Stitch com 25 projetos/177 telas, jornadas contratuais locais por pytest, 15 jornadas E2E verdes em `tests/e2e` e shells vivos em `all-in-one`, `all-in-one-business`, `all-in-one-user`, `all-in-one-services`, `all-in-one-health`, `all-in-one-mobility`, `valley-business` e `valley-rider` integrados a endpoints FastAPI | Ainda falta fechar os demais shells, ampliar Playwright E2E e sincronizar o Stitch remoto com seguranca. |
| Integracoes externas | 40% | Contratos, matriz versionada, adapters sandbox e endpoints administrativos locais existem, incluindo refund sandbox de Finance, liveness de Identity, rate limit de API Hub e o gateway financeiro de refund | Provedores reais dependem de credenciais/homologacao e testes de contrato externos. |
| Producao/compliance | 60% | `docs/COMPLIANCE.md`, matriz LGPD por modulo, fluxo de direitos do titular, contrato, worker local, fila PostgreSQL, agendamento seguro, runbook de backup/restore/DR e PrometheusRule/AlertmanagerConfig de retencao LGPD | Faltam aplicar os manifests no cluster real, mutacoes finais nos stores de dominio, DPIA assinada, pentest, carga, DR real e observabilidade produtiva. |
...
- Cobertura de geracao de eventos reais de criacao e transicao do recurso
...
- `all-in-one-services`: prestador, visita, orcamento, contrato, evidencia, slots e reserva.
...
| --- | ---: | --- | --- | --- |
| `identity` | 86% | Contrato, runtime, PostgreSQL especializado, cadastro/login/KYC/MFA/liveness E2E e container healthy | KYC/KYB/liveness reais | Homologar provedor KYC/KYB e ampliar testes negativos |
...
| `mobility` | 73% | Shell vivo com corridas, bilhetes QR/NFC, rotas e tarifas, integrado ao API Hub e validado em build e Playwright desktop/mobile | ETA, QR/NFC real e tarifas dinamicas | Levar jornada corrida e ticket para Playwright |
...
| `api_hub` | 84% | API keys/webhooks, OAuth2 client_credentials, SQL refinado, rotas gateway de API key/webhook, rate limit validado e testes de rate limit | OAuth2 real e testes de proxy com servicos vivos | Testar OAuth2 real, assinatura de webhooks de saida e rate limit com Redis real |
...
| `health` | 66% | Shell vivo com pacientes, agenda, prontuario, prescricoes e telemedicina, integrado ao API Hub e validado em build e Playwright desktop/mobile | Consentimento, prontuario e telemedicina reais | Implementar consentimento auditavel e prontuario |
