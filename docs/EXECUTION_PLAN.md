...
| Apps/frontend | 73% | 9 apps catalogados, catalogo Valley backend regionalizado, plano Stitch com 25 projetos/177 telas, jornadas contratuais locais por pytest e shells vivos em `all-in-one`, `all-in-one-business`, `all-in-one-user`, `valley-business` e `valley-rider` integrados a endpoints FastAPI | Ainda falta fechar os demais shells, ampliar Playwright E2E e sincronizar o Stitch remoto com seguranca. |
