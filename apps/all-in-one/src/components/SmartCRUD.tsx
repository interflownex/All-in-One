...
const LIVE_RESOURCE_ALIASES: Record<string, string> = {
  ...
  "identity:identityverifications": "identity_verifications",
  "identity:consentrecords": "consent_records",
  "business:companydocuments": "company_documents",
  "business:usercompanymemberships": "user_company_memberships",
  "business:catalogoffers": "catalog_offers",
  "permissions:userroles": "user_roles",
  "permissions:accesspolicies": "access_policies",
  "permissions:approvallimits": "approval_limits",
  "finance:ledgerentries": "ledger_entries",
  "finance:valleygoldledgerentries": "valley_gold_ledger_entries",
  "marketplace:pepitagrants": "pepita_grants",
  "stock:catalogproducts": "catalog_products",
  "stock:pricerules": "price_rules",
  "stock:supplierorders": "supplier_orders",
  "stock:discountquotes": "discount_quotes",
  "delivery:deliveryrequests": "delivery_requests",
  "delivery:insuranceoptions": "insurance_options",
  "riders:riderprofiles": "rider_profiles",
  "riders:riderdocuments": "rider_documents",
  "riders:riderreviews": "rider_reviews",
  "services:servicecontracts": "service_contracts",
  "mobility:farerules": "fare_rules",
  "jobs:employmentrecords": "employment_records",
  "jobs:resumedocuments": "resume_documents",
...
const SmartCRUD: React.FC<SmartCRUDProps> = ({ module, entity, type, title }) => {
  ...
  const [data, setData] = useState<any[]>(() =>
    API_HUB_TOKEN ? [] : demoRecordsFor(module, entity, title),
  );
  ...
  const [actionState, setActionState] = useState<"idle" | "running" | "completed" | "failed">(
    "idle",
  );
  const [formData, setFormData] = useState<Record<string, string | boolean>>(() =>
    formValuesFor(editingRecord, boundFields),
  );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const localRecords =
        storedRecords === null ? demoRecordsFor(module, entity, title) : JSON.parse(storedRecords);
      ...
      const filteredLocalRecords = localRecords.filter(
        (record: any) =>
          !query ||
          displayNameFor(record, title)
  ...
  const transitionResource = async (resourceId: string, action: string, reason: string) => {
    const response = await fetch(
      `${API_HUB_URL}${liveResourcePath}/${resourceId}/actions/${action}`,
      {
        method: "POST",
  ...
  const createLiveResource = async (path: string, payload: Record<string, unknown>) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        detail?.detail
          ? `Criacao retornou HTTP ${response.status}: ${detail.detail}`
          : `Criacao retornou HTTP ${response.status}.`,
      );
    }
    return response.json();
  };
  ...
  const runJourneyAction = async () => {
    ...
    try {
      if (module === "marketplace" && resourceType === "orders") {
        ...
        setData((items) =>
          items.map((item) =>
            item.id === resource.id ? { ...item, status: paid.status ?? "paid" } : item,
          ),
        );
        ...
        const completed = await transitionResource(
          resource.id,
          "complete",
          "entrega concluida pelo shell User",
        );
  ...
  const saveForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (API_HUB_URL && API_HUB_TOKEN) {
        ...
        const current =
          storedRecords === null
            ? demoRecordsFor(module, entity, title)
      ...
      setFormState("failed");
      setFormFeedback(
        error instanceof Error ? error.message : "Nao foi possivel salvar o registro.",
      );
    }
  };
  ...
  const deleteRecord = async (record: any) => {
    ...
    try {
      ...
      localStorage.setItem(
        localStorageKey,
        JSON.stringify(current.filter((item: any) => item.id !== record.id)),
      );
    ...
    return (
      <div className="container">
        <form className="neo-form neo-brutalism" onSubmit={saveForm}>
          <h2 style={{ marginBottom: "24px", color: "#236cff" }}>
            {title} - {editingRecord ? "Editar Registro" : "Novo Registro"}
          </h2>
          {fieldContract ? (
            <p className="notice" role="status">
              Persistência: <code>{fieldContract.physicalStorage}</code> · {boundFields.length}{" "}
              campos vinculados.
            </p>
          ) : null}
          {fieldContract?.immutable ? (
            <p className="notice" role="alert">
              Entidade imutável: use o fluxo transacional autorizado para criar registros.
            </p>
          ) : null}
          {boundFields.map((field) => {
              ...
              >
                ...
                {field.component === "textarea" ? (
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setFormData((current) => ({ ...current, [field.name]: event.target.checked }))
                    }
                  />
                ) : (
                  <input
          ...
          {formFeedback ? (
            <p
              className={`action-feedback ${formState === "failed" ? "error" : "success"}`}
              role="status"
            >
              {formFeedback}
            </p>
          ...
          >
            ...
            <button
              ...
              disabled={formState === "saving" || !fieldContract || fieldContract.immutable}
              style={{ padding: "10px 20px" }}
            >
              {formState === "saving" ? "Salvando..." : "Salvar Registro"}
            </button>
          </div>
        </form>
      </div>
  ...
  return (
    <div className="container" style={{ position: "relative" }}>
      ...
      <section className="hero">
        ...
        <p style={{ color: "#626b8e", fontSize: "1.1rem" }}>
      ...
      <div
        ...
        style={{
          background: "#fff",
          padding: "24px",
          border: "3px solid #11142a",
          boxShadow: "6px 6px 0px #11142a",
          marginBottom: "32px",
        }}
      >
        <div className="search-row search-row-crud">
          ...
          <button
            type="button"
            className="btn-secondary"
            onClick={() => navigate(`/${module}/${entity}-form`)}
      ...
      {canRunJourneyAction ? (
        <section
          className="action-panel neo-brutalism"
          aria-label="Acao de jornada User"
          style={{ background: "#fff", padding: "20px", marginBottom: "24px" }}
        >
          ...
          {actionFeedback ? (
            <p
              className={`journey-feedback ${actionState}`}
              style={{ marginTop: "12px", fontWeight: 800 }}
            >
              {actionFeedback}
            </p>
          ) : null}
      ...
      {isLiveApiHub && isJobsVacancyJourney ? (
        ...
        >
          <h2 style={{ fontSize: "1.25rem", fontWeight: 900, marginBottom: "12px" }}>
            Busca e notificacoes Jobs
          </h2>
          <ul style={{ margin: 0, paddingLeft: "20px", display: "grid", gap: "8px" }}>
            {notifications.length > 0 ? (
              notifications.map((message) => <li key={message}>{message}</li>)
            ) : (
          ...
          {postApplication ? (
            <div
              className="post-application-card"
              style={{ marginTop: "16px", padding: "16px", border: "2px solid #11142a" }}
            >
          ...
          {data.length > 0 ? (
            data.map((item: any) => (
              <div key={item.id} className="data-card neo-brutalism">
                {item.image ? (
                  <img className="data-card-media" src={item.image} alt="" loading="lazy" />
                ) : null}
                <div className="data-card-copy">
                  <h3 style={{ fontSize: "1.2rem", fontWeight: 800 }}>
                    {displayNameFor(item, title)}
                  </h3>
                  <p style={{ fontSize: "0.9rem", color: "#626b8e" }}>
                    ID: {item.id} | Criado em: {new Date(item.created_at).toLocaleDateString()}
                  </p>
                  {item.description ? (
                    <p className="data-card-description">{item.description}</p>
                  ) : null}
                  {fieldContract?.listFields.map((fieldName) => {
                    ...
                    return field && value !== undefined && value !== null ? (
                      <p key={fieldName} className="data-card-description">
                ...
                <div className="data-card-actions">
                  <span
                    className="badge"
                    style={{
                      background: item.status === "Ativo" ? "#eef1ff" : "#fef3c7",
                      color: item.status === "Ativo" ? "#1a6fb3" : "#92400e",
                      padding: "6px 12px",
                      borderRadius: "4px",
                      fontWeight: 700,
                    }}
      ...
      {selectedMedia ? (
        ...
        >
          <section
            className="modal-content media-detail-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="media-detail-title"
          >
            <div className="modal-header">
              <h2 id="media-detail-title">{displayNameFor(selectedMedia, title)}</h2>
              <button
                type="button"
                className="close-btn"
            ...
            <div className="modal-body">
              {selectedMedia.video ? (
                ...
                <img
                  ...
                  alt={displayNameFor(selectedMedia, title)}
                />
              )}
              ...
              <dl className="detail-metadata">
                ...
                <div>
                  <dt>Regiao</dt>
                  <dd>{selectedMedia.region ?? "Brasil"}</dd>
                </div>
                <div>
                  <dt>Categoria</dt>
                  <dd>{selectedMedia.category ?? "Padrao"}</dd>
                </div>
              </dl>
            </div>
          </section>
