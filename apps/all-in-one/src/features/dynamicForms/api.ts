...
const TENANT_ID =
  (import.meta as any).env?.VITE_TENANT_ID ?? "00000000-0000-4000-8000-000000000101";
const ACTOR_ID =
  (import.meta as any).env?.VITE_ACTOR_USER_ID ?? "00000000-0000-4000-8000-000000000102";
...
async function request<T>(path: string, init?: RequestInit, idempotencyKey?: string): Promise<T> {
  const response = await fetch(`${BASE}${path}`, {
    ...init,
    headers: { ...requestHeaders(idempotencyKey), ...init?.headers },
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok)
    throw new Error(payload.detail ?? "Não foi possível concluir a operação no backend.");
  return payload as T;
}
...
export async function saveDocument(
  ...
  if (!definitionId || !versionId)
    throw new Error("Backend não retornou os identificadores do formulário.");
  await request(`/versions/${versionId}/blueprint`, {
    method: "PUT",
    body: JSON.stringify({
...
export async function requestHomologation(versionId: string): Promise<void> {
  await request(
    `/versions/${versionId}/homologations`,
    {
      method: "POST",
      body: JSON.stringify({
        checklist: { blueprint_validated: true, preview_reviewed: true, no_physical_binding: true },
      }),
    },
