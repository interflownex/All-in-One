  ...
  blocks: [
    {
      ...
      description: "Informações essenciais para o cadastro.",
      width: 12,
      collapsible: false,
...
export const saveLocalDocument = (document: BuilderDocument) =>
...
export function createField(
  ...
  const validations: BuilderValidation[] = catalog.mandatory_validations.map((type) => ({
    ...
    parameters: {},
    message_pt_br: validationMessage[type] ?? "Valor inválido.",
    severity: type === "required" ? "blocking" : "error",
    condition: null,
    run_frontend: true,
    run_backend: true,
    status: "active",
  ...
  const field: BuilderField = {
    id: fieldId,
    block_id: blockId,
    field_catalog_id: catalog.id,
    field_binding_id: binding.id,
    component,
    label,
    help_text: "",
    placeholder: "",
    required: catalog.mandatory_validations.includes("required"),
    read_only: false,
    hidden: false,
    display_order: order,
    width: 12,
    ...
    permissions: {},
    visibility_rule_id: null,
    validation_ids: validations.map((item) => item.id),
    audit_policy: { sensitivity: catalog.sensitivity },
  };
...
export const demoCatalog: CatalogField[] = [
  {
    id: "demo-legal-name",
    domain: "business",
    logical_entity: "companies",
    logical_field: "legal_name",
    data_type: "string",
  ...
  },
  {
    ...
    domain: "business",
    logical_entity: "companies",
    logical_field: "document_number",
    data_type: "string",
    description: "CNPJ",
    allowed_components: ["text"],
    mandatory_validations: ["required", "document_checksum"],
    allowed_calculations: [],
  ...
  },
  {
    ...
    domain: "business",
    logical_entity: "companies",
    logical_field: "responsible_email",
    data_type: "string",
    description: "E-mail responsável",
    allowed_components: ["email"],
    mandatory_validations: ["required", "email"],
    allowed_calculations: [],
    sensitivity: "personal",
    status: "active",
  },
  {
    ...
    domain: "business",
    logical_entity: "companies",
    logical_field: "share_capital",
    data_type: "decimal",
    description: "Capital social",
    allowed_components: ["currency"],
...
export const demoBindings: FieldBinding[] = demoCatalog.map((field) => ({
  id: `binding-${field.id}`,
  field_catalog_id: field.id,
  command: "business.company.update",
  api: "/business/resources/companies",
  dto: "CompanyPatch",
  logical_path: `payload.${field.logical_field}`,
  status: "active",
}));
