...
const endpoints = [
  {
    label: "Prestadores",
    path: "/services/resources/providers",
    fallback: "4 prestadores elegiveis",
  },
  {
    label: "Contratos",
    path: "/services/resources/service_contracts",
    fallback: "2 contratos em andamento",
  },
  { label: "Escrows", path: "/finance/resources/escrows", fallback: "1 escrow simulado" },
  {
    label: "Evidencias",
    path: "/document/resources/documents",
    fallback: "3 evidencias previstas",
  },
];
...
async function transitionContract(
  ...
  const response = await fetch(
    `${API_HUB_URL}/services/resources/service_contracts/${contractId}/actions/${action}`,
    {
      method: "POST",
      headers: {
...
function App() {
  ...
  const [journeyMessage, setJourneyMessage] = useState(
    "Pronto para aceitar contrato, reter escrow e anexar evidencia.",
  );
        ...
        try {
          ...
          if (endpoint.path.endsWith("/providers") && data[0]) {
            setEvidence((current) => ({
              ...current,
              provider: String(
                data[0].payload?.name ?? data[0].payload?.category ?? "Prestador validado",
              ),
            }));
          }
  ...
  async function completeServiceJourney() {
    ...
    try {
      ...
      setContract(held);
      const completed = await transitionContract(
        contract.id,
        "complete",
        "servico executado com evidencia",
        {
      ...
      setEvidence((current) => ({
        ...current,
        followUp:
          "Pos-atendimento criado: liberar escrow apos conferencia da evidencia e registrar satisfacao do cliente.",
      }));
  ...
  return (
    <main className="shell">
      <section className="hero">
        ...
        <p>
          Shell dedicado para prestadores e operadores acompanharem a jornada de ponta a ponta antes
          da expansao Playwright.
        </p>
      </section>
      ...
      <section className="action-panel" aria-label="Acao de jornada Services">
        <div>
          ...
          <p>
            Usa o primeiro contrato retornado pelo API Hub para simular aceite, escrow retido e
            entrega com evidencia auditavel.
          </p>
        </div>
        <dl>
          <div>
        ...
        <button
          type="button"
          onClick={completeServiceJourney}
      ...
      <section className="action-panel" aria-label="Pos-atendimento Services">
        <div>
          ...
          <p>
            Consolida metadados operacionais do API Hub para acompanhar a liberacao financeira e a
            prova de execucao sem expor documentos.
          </p>
        </div>
        <dl>
          <div>
          ...
          <div>
            <dt>Evidencia</dt>
            <dd>
              {evidence.documents > 0 ? "Evidencia documental disponivel" : "Evidencia pendente"}
            </dd>
          </div>
          <div>
            <dt>Retorno</dt>
