...
const FeaturedItems = () => {
  ...
  return (
    <div className="container">
      <section className="hero" style={{ background: '#126b45', color: '#fff', padding: '60px 40px', borderRadius: '8px', marginBottom: '40px' }}>
        <h1 style={{ color: '#fff' }}>Bem-vindo ao Valley Business</h1>
        <p style={{ color: '#e2f2ea' }}>Gerencie sua operação corporativa com o poder do ecossistema All-in-One.</p>
      </section>
      ...
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginBottom: '40px' }}>
        {[
          ...
          <div key={s.label} className="metric-card neo-brutalism" style={{ padding: '20px', background: '#fff' }}>
            <div className="label" style={{ fontSize: '0.8rem', color: '#536159', fontWeight: 800 }}>{s.label}</div>
            <div className="value" style={{ fontSize: '1.8rem', fontWeight: 900, color: '#126b45' }}>{s.value}</div>
          </div>
        ))}
      </div>

      <div className="featured-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        {items.map(item => (
          <div key={item.id} className="offer-card neo-brutalism" style={{ display: 'flex', flexDirection: 'column', background: '#fff' }}>
            <div className="media-container" style={{ position: 'relative', height: '180px', overflow: 'hidden', borderRadius: '4px', marginBottom: '12px', border: '2px solid #17211c' }}>
            ...
            <button className="btn-primary" style={{ padding: '10px' }}>Acessar Módulo</button>
          </div>
        ))}
      </div>
    </div>
