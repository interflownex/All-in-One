...
const CompaniesForm: React.FC = () => {
  ...
  const activeCount = recommendations.filter((module) =>
    ["mandatory", "active"].includes(module.state),
  ).length;
  const recommendedCount = recommendations.filter(
    (module) => module.state === "recommended",
  ).length;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (businessModulesApiEnabled) {
        ...
        setSavedMessage(
          `Cadastro enviado ao back-end. ${result.modules.length} módulos foram classificados, aplicados e auditados para a empresa.`,
        );
        return;
      }
      setSavedMessage(
        "Cadastro validado localmente. Configure VITE_API_HUB_URL e VITE_API_HUB_TOKEN para aplicar a classificação e os módulos no back-end Business.",
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Não foi possível aplicar os módulos no back-end. Seus dados foram preservados.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="container">
      <section className="hero a1-module-hero" style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "#536159", fontSize: "1.05rem" }}>
          Informe os dados principais da empresa. O sistema classifica o perfil operacional e
          seleciona automaticamente os módulos adequados, ocultando o que não fizer sentido para a
          operação inicial.
        </p>
      </section>
      ...
      >
        ...
        >
          ...
          >
            <label style={labelStyle}>
              ...
              <input
                ...
                required
                placeholder="Ex.: Brasil Desconto Tecnologia Ltda."
                style={fieldStyle}
              />
            </label>
            <label style={labelStyle}>
              ...
              <input
                ...
                placeholder="Ex.: Brasil Desconto"
                style={fieldStyle}
              />
            </label>
            <label style={labelStyle}>
            ...
            <label style={labelStyle}>
              ...
              <select
                ...
                onChange={(event) => update("businessKind", event.target.value as BusinessKind)}
                style={fieldStyle}
              >
            ...
            <label style={labelStyle}>
              ...
              <input
                ...
                onChange={(event) => update("cnaePrimary", event.target.value)}
          ...
          >
            {[
              ...
              <label
                key={key}
                style={{ display: "flex", gap: 10, alignItems: "center", fontWeight: 700 }}
              >
                <input
                  type="checkbox"
          ...
          <label style={{ ...labelStyle, marginBottom: 20 }}>
            ...
            <textarea
              ...
              placeholder="Explique o que a empresa vende, entrega, produz ou administra."
        ...
        >
          ...
          <p style={{ color: "#536159", fontSize: 13, marginBottom: 12 }}>
            {businessModulesApiEnabled
              ? "Integração com back-end Business habilitada."
              : "Prévia local ativa; back-end será usado quando o API Hub estiver configurado."}
          </p>
          <div
            style={{
              display: "grid",
          ...
          >
            <strong>
              {activeCount}
              <br />
              <span style={{ color: "#536159", fontSize: 12 }}>ativos</span>
            </strong>
            <strong>
              {recommendedCount}
              <br />
              <span style={{ color: "#536159", fontSize: 12 }}>recomendados</span>
            </strong>
            <strong>
              {hiddenCount}
              <br />
              <span style={{ color: "#536159", fontSize: 12 }}>ocultos</span>
            </strong>
          </div>
            ...
            {recommendations.map((module) => (
              <div
                ...
                style={{
                  border: "2px solid #17211c",
                  padding: 12,
                  background: module.state === "hidden" ? "#f9fafa" : "#fff",
                }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                  <strong>{module.titlePtBr}</strong>
                  <span style={{ color: stateTone[module.state], fontWeight: 900 }}>
                    {stateLabel[module.state]}
                  </span>
                </div>
                <p style={{ margin: "8px 0 0", color: "#536159", fontSize: 13 }}>
                  {module.explanationPtBr}
                </p>
              </div>
            ))}
          </div>
