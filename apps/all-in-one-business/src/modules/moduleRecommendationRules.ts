...
function buildExplanation(moduleSlug: string, state: ModuleState, businessKind: BusinessKind) {
  ...
  if (state === 'mandatory') return `${moduleName} e obrigatorio para garantir identidade, empresa, permissoes e auditoria da operacao.`;
  if (state === 'active') return `${moduleName} foi ativado automaticamente porque combina com o perfil operacional ${businessKind}.`;
  if (state === 'recommended') return `${moduleName} e recomendado, mas pode aguardar configuracao manual pela empresa.`;
  if (state === 'hidden') return `${moduleName} ficara oculto na navegacao inicial para reduzir complexidade sem apagar dados.`;
