...
const endpoints = [
  ...
  { label: "Agendas", path: "/health/resources/appointments", fallback: "3 consultas previstas" },
  {
    ...
    path: "/identity/resources/consents",
    fallback: "1 consentimento pendente",
  },
...
async function transitionAppointment(
  ...
  const response = await fetch(
    `${API_HUB_URL}/health/resources/appointments/${appointmentId}/actions/${action}`,
    {
      method: "POST",
      headers: {
...
function App() {
  ...
  const [journeyMessage, setJourneyMessage] = useState(
    "Pronto para aprovar consulta e registrar atendimento concluido.",
  );
        ...
        try {
          ...
          if (endpoint.path.endsWith("/patients") && data[0]) {
            setGovernance((current) => ({
              ...current,
              patientSummary: String(
                data[0].payload?.name ?? data[0].payload?.health_identifier ?? "Paciente protegido",
              ),
            }));
          }
  ...
  async function completeHealthJourney() {
    ...
    try {
      const approved = await transitionAppointment(
        appointment.id,
        "approve",
        "consulta autorizada pelo Playwright",
      );
      setAppointment(approved);
      const completed = await transitionAppointment(
        appointment.id,
        "complete",
        "atendimento concluido com prontuario protegido",
      );
      setAppointment(completed);
      setGovernance((current) => ({
        ...current,
        followUp:
          "Retorno pos-consulta criado: revisar prontuario protegido e manter consentimento LGPD ativo.",
      }));
  ...
  return (
    <main className="shell">
      <section className="hero">
        ...
        <p>
          Shell dedicado para validar a jornada paciente, agenda, prontuario e consulta antes de
          conectar telemedicina e provedores reais.
        </p>
      </section>
      ...
      <section className="action-panel" aria-label="Acao de jornada Health">
        <div>
          ...
          <p>
            Usa a primeira agenda retornada pelo API Hub para validar aprovacao clinica com MFA e
            conclusao do atendimento sem expor dado sensivel.
          </p>
        </div>
        <dl>
          <div>
      ...
      <section className="action-panel" aria-label="Governanca clinica Health">
        <div>
          ...
          <p>
            Consolida apenas metadados operacionais retornados pelo API Hub, mantendo dados
            sensiveis fora da interface de acompanhamento.
          </p>
        </div>
        <dl>
          <div>
          ...
          <div>
            ...
            <dd>
              {governance.consents > 0 ? "Consentimento verificado" : "Aguardando consentimento"}
            </dd>
          </div>
          <div>
            <dt>Prontuario</dt>
            <dd>
              {governance.documents > 0
                ? "Prontuario protegido disponivel"
                : "Documento clinico pendente"}
            </dd>
          </div>
          <div>
            <dt>Retorno</dt>
