...
export function HomeView({ session, setNotice }: ViewProps) {
  ...
  const load = useCallback(async (append = false) => {
    ...
    params.append("offset", String(append ? offers.length : 0));
  ...
  }, [category, offers.length, query, setNotice]);
  ...
  const submitSearch = (event: FormEvent) => { event.preventDefault(); void load(false); };
  ...
  return <section><SectionHeader title='Descobrir no Valley' subtitle='Busca regional, categorias, ofertas verificadas e ações sincronizadas.' />
    ...
    {loading && <StateCard text='Sincronizando catálogo...' />}{error && <StateCard text={error} tone='error' actionLabel='Tentar novamente' onAction={() => void load(false)} />}{!loading && !error && !offers.length && <StateCard text='Nenhuma oferta encontrada para estes filtros.' />}
