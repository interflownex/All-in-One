...
const OrdersDrawerContent: React.FC<{ onClose: () => void; token: string }> = ({
  onClose,
  token,
}) => {
                ...
                {items.map((item) => (
                  <article key={`${item.kind}-${item.id}`} className="order-card">
                    ...
                    <div className="order-details">
                      ...
                      {item.amount_brl && (
                        <p className="price">
                          Valor: R$ {Number(item.amount_brl).toFixed(2).replace(".", ",")}
                        </p>
                      )}
                      {item.scheduled_at && (
                        <p className="schedule">
                          Agendado para: {new Date(item.scheduled_at).toLocaleString()}
                        </p>
                      )}
                      {["paid", "accepted", "in_progress", "delivered", "completed"].includes(
                        item.status,
                      ) && (
                        <button
