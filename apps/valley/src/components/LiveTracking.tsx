...
const WEBSOCKET_URL =
  import.meta.env.VITE_API_HUB_URL?.replace("http", "ws") || "wss://all-in-one-api-hub.web.app";
    ...
    const startLocalSimulation = () => {
      ...
      fallbackId = setInterval(() => {
        ...
        setLogs((prev) =>
          [
            ...prev,
            `[SISTEMA] GPS Atualizado: Lat ${next.lat.toFixed(4)}, Lng ${next.lng.toFixed(4)}`,
          ].slice(-6),
        );
      }, 3500);
    };
      ...
      try {
        ...
        if (data.status === "connected") {
          ...
          setLogs((prev) =>
            [
              ...prev,
              `[SISTEMA] GPS Atualizado: Lat ${parsedUpdate.lat.toFixed(4)}, Lng ${parsedUpdate.lng.toFixed(4)}`,
            ].slice(-5),
          );
        }
      } catch {
        setLogs((prev) =>
          [...prev, "[SISTEMA] Pacote de rastreio descartado por formato invalido."].slice(-6),
        );
      }
    };
