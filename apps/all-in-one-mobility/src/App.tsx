...
function App() {
      ...
      endpoints.map(async endpoint => {
        try {
          ...
          if (endpoint.path.endsWith('/rider_profiles') && data[0]) {
            setOperations(current => ({
              ...current,
              rider: String(data[0].payload?.name ?? data[0].payload?.wallet_id ?? 'Rider elegivel'),
            }))
          }
          if (endpoint.path.endsWith('/wallets') && data[0]) {
            setOperations(current => ({
              ...current,
              wallet: String(data[0].payload?.label ?? data[0].payload?.wallet_type ?? 'Wallet vinculada'),
            }))
          }
          return {
            label: endpoint.label,
  ...
  async function completeMobilityJourney() {
    ...
    try {
      ...
      setOperations(current => ({
        ...current,
        receipt: 'Comprovante pos-corrida criado: rota concluida, ticket usado e wallet pronta para conciliacao.',
      }))
    ...
    <main className="shell">
      ...
      <section className="action-panel" aria-label="Pos-corrida Mobility">
        <div>
          ...
          <p>
            Consolida rota, operador, ticket tokenizado e wallet para suporte,
            auditoria antifraude e conciliacao financeira sem expor token bruto.
          </p>
        </div>
        <dl>
          <div>
