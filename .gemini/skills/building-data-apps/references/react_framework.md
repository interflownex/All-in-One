...
- _State Normal:_ Table takes `w-full`.
- _State Selected:_ Table shrinks to `w-2/3`, side panel slides/fades in as `w-1/3`. Add `transition-all duration-300` to make the shift smooth.
...
- **Toolbar:** Above the table, place filtering actions (e.g., "Filter",
  ...
  - _Filtering Logic:_ Do not delete raw data when filtering. Use local
