...
-> A`) is not supported.
  - **Convergent Paths (Diamonds)**: Multiple distinct directed paths
    ...
    (descendant) node are not supported. _(Note: This is distinct from
    undirected cycles; if the paths have different directions, it may be
    supported, but a strict directed diamond pattern is disallowed.)_
...
- **Incorrect**:
  ...
  _Throws error_: `Returning expressions of type MEASURE is not allowed.`
...
- **GQL Limitation**: Columns declared as `MEASURE` in the DDL _cannot_ be
