...
from generate_data_audit_inventory import (
    ROOT,
    discover_logical_rules,
    discover_physical_model,
)
...
def component_for(name: str, physical_type: str) -> tuple[str, str]:
    ...
    if any(
        token in lowered
        for token in ("numeric", "decimal", "integer", "bigint", "smallint", "real")
    ):
    ...
    for entity in logical:
        ...
        for field in tables.get(target, []):
            if field.physical_name in SYSTEM_FIELDS:
                continue
            component, logical_type = component_for(
                field.physical_name, field.physical_type
            )
            ...
            fields.append(
                {
                    ...
                    "validation": "required + backend domain rule"
                    if required
                    else "backend domain rule",
                    "evidence": field.evidence,
                }
            )
        if not fields:
            raise RuntimeError(
                f"Entidade sem campo de interface elegível: {module}.{resource} ({target})"
            )
        contracts[f"{module}:{resource}"] = {
            "module": module,
            "resource": resource,
            "endpoint": f"/{module}/resources/{resource}",
            "createDto": "ResourceCreate",
            "patchDto": "ResourcePatch",
            "physicalStorage": target,
            "immutable": bool(entity["immutable"]),
            "sensitive": bool(entity["sensitive"]),
            "fields": fields,
            "listFields": [item["name"] for item in fields[:6]],
        ...
        json.dumps(
            {
                ...
                "fields": sum(len(item["fields"]) for item in contracts.values()),
            },
            ensure_ascii=False,
        )
    )
