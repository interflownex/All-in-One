...
def deploy_apk(project_id: str, bucket_name: str, object_name: str) -> str:
    ...
    if not APK_PATH.exists():
        raise FileNotFoundError(
            f"APK nao encontrado em: {APK_PATH}. Certifique-se de executar o build antes."
        )

    print(f"[*] Verificando existencia do bucket gs://{bucket_name}...")
    buckets_raw = run_command(
        [
            gcloud,
            "storage",
            "buckets",
    ...
    print(
        f"[*] Fazendo upload do APK: {APK_PATH} -> gs://{bucket_name}/{object_name}..."
    )
    run_command(
        [
            gcloud,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Realiza o deploy do APK para o Google Cloud Storage e gera link publico."
    )
    parser.add_argument(
        "--project",
        help="ID do projeto Google Cloud (opcional se GOOGLE_CLOUD_PROJECT estiver definido)",
    )
    parser.add_argument(
        "--bucket",
        default=DEFAULT_BUCKET,
        help=f"Nome do bucket (padrao: {DEFAULT_BUCKET})",
    )
    parser.add_argument(
        "--name",
        default=DEFAULT_OBJECT,
        help=f"Nome do objeto no bucket (padrao: {DEFAULT_OBJECT})",
    )
    ...
    if not project_id:
        print(
            "Erro: ID do projeto nao definido. Use --project ou defina GOOGLE_CLOUD_PROJECT.",
            file=sys.stderr,
        )
        return 1
