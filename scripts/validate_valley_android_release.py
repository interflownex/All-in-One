...
def require(text: str, marker: str, source: Path, errors: list[str]) -> None:
    if marker not in text:
        errors.append(
            f"{source.relative_to(ROOT)}: marcador obrigatorio ausente: {marker}"
        )
...
def validate() -> list[str]:
    ...
    play_integrity_policy_path = (
        ROOT / "config" / "security" / "valley_play_integrity_policy.json"
    )
    ...
    kotlin_sources = "\n".join(
        path.read_text(encoding="utf-8") for path in source_root.rglob("*.kt")
    )
    if re.search(r'putString\(\s*"(?:token|password|refresh_token)"', kotlin_sources):
        errors.append(
            "credencial sensivel persiste em SharedPreferences sem envelope criptografado"
        )
    if "buildDemoSession(" in kotlin_sources:
        errors.append(
            "aplicativo Android ainda aceita sessao local simulada quando o backend falha"
        )
    for marker in (
        "/auth/refresh",
        "/auth/logout",
        ...
        if marker not in kotlin_sources:
            errors.append(
                f"cliente Android nao implementa contrato de sessao obrigatorio: {marker}"
            )
    for marker in (
        ...
        if marker not in kotlin_sources:
            errors.append(
                f"WebView Android sem requisito de memoria/ciclo de vida: {marker}"
            )
    ...
    for marker in (
        ...
        "testProductionDebugUnitTest lintProductionDebug assembleProductionDebug",
    ...
    for marker in (
        ...
        if marker not in web_sources:
            errors.append(
                f"frontend Valley sem requisito de catálogo/performance: {marker}"
            )

    embedded_web = ANDROID / "app" / "src" / "main" / "assets" / "valley"
    embedded_javascript = "\n".join(
        path.read_text(encoding="utf-8") for path in embedded_web.rglob("*.js")
    )
    if "VITE_VALLEY_ALLOW_DEMO" in embedded_javascript:
        errors.append(
            "bundle Android permite alterar o modo demonstrativo em tempo de execução"
        )
    ...
    if embedded_pngs:
        errors.append(
            "bundle Android ainda contém logotipos PNG não otimizados: "
            + ", ".join(embedded_pngs)
        )
    oversized_brand = sorted(
        path.name
        for path in embedded_brand.glob("*.webp")
