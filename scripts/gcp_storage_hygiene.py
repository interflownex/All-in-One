...
CLOUDBUILD_SOURCE_RETENTION_DAYS = int(os.getenv("CLOUDBUILD_SOURCE_RETENTION_DAYS", "30"))
...
def clean_cloud_build_bucket() -> None:
    logging.info("Iniciando limpeza segura no Cloud Storage (preservando pacotes fonte recentes)...")
    objects = run_json(["gcloud", "storage", "ls", f"gs://{PROJECT_ID}_cloudbuild/source/*.tgz"])
    ...
    logging.info("Removendo %s pacote(s) fonte antigo(s) fora da janela de retencao.", len(removable))
