...
PRODUCT_UNITS_TAX_MODEL = (
    ROOT / "config" / "data_audit" / "product_units_tax_model_proposal.json"
)
FIELD_CLASSIFICATION_POLICY = (
    ROOT / "config" / "data_audit" / "field_classification_policy.json"
)
AUDIT_TRACEABILITY_POLICY = (
    ROOT / "config" / "data_audit" / "audit_traceability_policy.json"
)
STITCH_TEMPLATE_COORDINATE = (
    ROOT / "config" / "stitch" / "template_project_coordinate.json"
)
MASTER_MEMO = (
    ROOT / "docs" / "MEMORANDO_MESTRE_GEMINI_VARREDURA_DADOS_FORMULARIOS_ALL_IN_ONE.md"
)
...
def parse_field(
    schema: str, table: str, definition: str, evidence: str
) -> Field | None:
    normalized = " ".join(definition.split())
    if not normalized or re.match(
        r"^(CONSTRAINT|PRIMARY|FOREIGN|UNIQUE|CHECK|EXCLUDE)\b", normalized, re.I
    ):
        return None
    ...
    default_match = re.search(
        r"\bDEFAULT\s+(.+?)(?=\s+(?:NOT\s+NULL|NULL|PRIMARY\s+KEY|UNIQUE|REFERENCES|CHECK)\b|$)",
        remainder,
        re.I,
    )
    reference_match = re.search(
        r"\bREFERENCES\s+([\w.\"]+)\s*\(([^)]+)\)", remainder, re.I
    )
    ...
    return Field(
        ...
        and "PRIMARY KEY" not in remainder.upper(),
        ...
        reference=".".join(part.strip('" ') for part in reference_match.groups())
        if reference_match
        else "",
        lgpd=lgpd,
...
def discover_physical_model() -> tuple[
    set[str], dict[str, list[Field]], list[str], list[str]
]:
    schemas: set[str] = set()
    ...
    alter_table = re.compile(
        r"ALTER\s+TABLE\s+([\w\"]+)\.([\w\"]+)\s+(.*?);", re.I | re.S
    )
    ...
    for path in migrations:
        ...
        for match in create_table.finditer(text):
            ...
            for definition in split_definitions(body):
                ...
                evidence = (
                    f"{relative}:{line_number(text, body_offset + max(local, 0))}"
                )
        ...
        for match in alter_table.finditer(text):
            ...
            for column in re.finditer(
                r"ADD\s+COLUMN\s+(?:IF\s+NOT\s+EXISTS\s+)?([\w\"]+)\s+(.+?)(?=,\s*ADD\s+COLUMN|$)",
                body,
                re.I | re.S,
            ):
                ...
                evidence = (
                    f"{relative}:{line_number(text, match.start(3) + column.start())}"
                )
                field = parse_field(schema, table, definition, evidence)
                if field and all(
                    existing.physical_name != name for existing in tables.get(key, [])
                ):
...
def discover_mongodb_model() -> list[dict[str, object]]:
    ...
    for index, match in enumerate(starts):
        ...
        required = (
            set(re.findall(r'"([^"]+)"', required_match.group(1)))
            if required_match
            else set()
        )
            ...
            rows.append(
                {
                    ...
                    "sensitive_declared": field_name
                    in contract["collections"][collection]["sensitive_fields"],
                    "lgpd": lgpd,
    ...
    return [
        {
            "technology": "Redis 7",
            "database": "0",
            "key_pattern": "rate_limit:{ip}",
            "purpose": "Contador de requisições por endereço IP no API Hub",
            "owner": "api_hub",
            "operations": ["GET", "INCR", "EXPIRE"],
            "value_type": "integer",
            "ttl_seconds": 60,
            "limit": 100,
            "source_of_truth": False,
            "data_classification": "identificador técnico pseudônimo",
            "encryption": "trânsito/rede privada dependem do ambiente; não comprovado em runtime",
            "retention": "TTL deslizante de 60 segundos",
            "failure_mode": "fail-open quando cliente ou Redis falha",
            "backup_restore": "não requerido para cache; reconstruído pelo tráfego",
            "runtime_verified": False,
            "evidence": source_evidence(
                "modules/api_hub/main.py", 'key = f"rate_limit:{ip}"'
            ),
            "infrastructure_evidence": [
                source_evidence(
                    "infra/docker/docker-compose.yml", "image: redis:7-alpine"
                ),
                source_evidence(
                    "infra/terraform/redis.tf", 'redis_version = "REDIS_7_0"'
                ),
            ],
        }
    ]
...
def discover_object_storage_catalog() -> list[dict[str, object]]:
    return [
        {
            "store": "private_document_vault",
            "provider": "filesystem privado local/fallback",
            "container": "ALL_IN_ONE_PRIVATE_DOCUMENT_DIR",
            "object_pattern": "ctps/{sha256_prefix}/{sha256}.pdf.aesgcm",
            "purpose": "PDFs CTPS e evidências privadas",
            "owner": "jobs/shared",
            "access": "backend PrivateDocumentStore",
            "classification": "crítica",
            "encryption": "AES-256-GCM com AAD SHA-256",
            "retention": "não implementada no store; política externa necessária",
            "versioning": "deduplicação pelo hash; sem versionamento nativo",
            "backup_restore": "política declara snapshot diário versionado e teste trimestral",
            "public": False,
            "runtime_verified": False,
            "evidence": source_evidence(
                "modules/shared/private_documents.py", 'URI_PREFIX = "private://jobs/"'
            ),
        },
        {
            "store": "document_domain_references",
            "provider": "private_vault|gcs_kms|s3_kms",
            "container": "storage_bucket por documento",
            "object_pattern": "storage_key versionada",
            "purpose": "Documentos, versões, OCR e assinatura",
            "owner": "document",
            "access": "serviço document com autorização",
            "classification": "crítica conforme documento",
            "encryption": "KMS obrigatório por contrato",
            "retention": "retention_policies por domínio; enforcement runtime não comprovado",
            "versioning": "versions append-only",
            "backup_restore": "RPO 60 min, RTO 240 min na política declarada",
            "public": False,
            "runtime_verified": False,
            "evidence": source_evidence(
                "modules/document/CONTRACT.md",
                "storage_key` e `storage_bucket` devem apontar para cofre privado",
            ),
        },
        {
            "store": "mobile_public_artifacts",
            "provider": "Google Cloud Storage",
            "container": "all-in-one-public-artifacts",
            "object_pattern": "valley-latest.apk",
            "purpose": "Distribuição pública de artefato Android",
            "owner": "release engineering",
            "access": "allUsers roles/storage.objectViewer",
            "classification": "pública",
            "encryption": "padrão do provedor; não comprovado",
            "retention": "não declarada",
            "versioning": "nome latest sobrescrevível; versionamento de bucket não declarado",
            "backup_restore": "não declarado; artefato pode ser reconstruído",
            "public": True,
            "runtime_verified": False,
            "evidence": source_evidence(
                "scripts/deploy_mobile_artifacts.py",
                'DEFAULT_BUCKET = "all-in-one-public-artifacts"',
            ),
        },
        {
            "store": "terraform_state",
            "provider": "Google Cloud Storage",
            "container": "all-in-one-tfstate",
            "object_pattern": "terraform/state/**",
            "purpose": "Estado da infraestrutura Terraform",
            "owner": "plataforma",
            "access": "IAM do bucket; não inventariado neste artefato",
            "classification": "operacional sensível",
            "encryption": "padrão do provedor; CMEK não declarada no backend",
            "retention": "não declarada",
            "versioning": "não declarado no código",
            "backup_restore": "não declarado",
            "public": False,
            "runtime_verified": False,
            "evidence": source_evidence(
                "infra/terraform/provider.tf", 'bucket = "all-in-one-tfstate"'
            ),
        },
    ]
...
def discover_browser_storage_catalog() -> list[dict[str, object]]:
    definitions = [
        ...
        (
            "sessionStorage",
            "valley.session.email",
            "email injetado pelo shell Android",
            "dado pessoal",
            "até clear/logout",
            "apps/valley-android/app/src/main/java/com/example/valley/ui/main/MainScreen.kt",
            "valley.session.email",
        ),
        (
            "sessionStorage",
            "valley.session.source",
            "origem da sessão injetada",
            "metadado técnico",
            "até clear/logout",
            "apps/valley-android/app/src/main/java/com/example/valley/ui/main/MainScreen.kt",
            "valley.session.source",
        ...
        (
            "localStorage",
            "valley.session.mode",
            "ativa modo de demonstração",
            "configuração",
            "sem TTL; remoção explícita",
        ...
        (
            ...
            "valley.demo.users",
            "usuários simulados",
            "dado de demonstração potencialmente pessoal",
            "sem TTL",
            "apps/valley/src/lib/valleyPlatform.ts",
            "valley.demo.users",
        ),
        ...
        (
            ...
            "valley.demo.orders",
            "pedidos simulados",
            "dado transacional de demonstração",
            "sem TTL",
            "apps/valley/src/lib/valleyPlatform.ts",
            "valley.demo.orders",
        ),
        ...
        (
            "localStorage",
            "valley.demo.support",
            "casos de suporte simulados",
            "conteúdo de demonstração potencialmente pessoal",
            "sem TTL",
            "apps/valley/src/lib/valleyPlatform.ts",
            "valley.demo.support",
        ),
        (
            "localStorage",
            "valley.catalog.v1.{query}",
            "cache de páginas do catálogo",
            "catálogo público e parâmetros de busca",
            "timestamp cachedAt gravado; expiração não comprovada",
            "apps/valley/src/lib/valleyPlatform.ts",
            "valley.catalog.v1.",
        ),
        (
            "localStorage",
            "all-in-one:{module}:{resourceType}",
            "fallback CRUD local por recurso",
            "pode conter dados de qualquer módulo",
            "sem TTL",
    ...
    return [
        {
            ...
            "retention": retention,
            "owner": "frontend indicado pela evidência",
            "operations": ["GET", "SET", "REMOVE/CLEAR quando implementado"],
            "encryption": "não criptografado pela aplicação",
            "source_of_truth": False,
            "risk": "acessível a JavaScript/XSS; não usar como fonte oficial nem persistir segredo além da sessão necessária",
            "runtime_verified": False,
            "evidence": source_evidence(relative, needle),
        }
    ...
    for path in sorted((ROOT / "modules").rglob("*.py")):
        ...
        for node in tree.body:
            if not isinstance(node, ast.ClassDef) or not any(
                annotation_name(base).endswith("BaseModel") for base in node.bases
            ):
                continue
            models[node.name] = [
                item.target.id
        ...
        for node in ast.walk(tree):
            ...
            for decorator in node.decorator_list:
                ...
                if not isinstance(route_node, ast.Constant) or not isinstance(
                    route_node.value, str
                ):
                    continue
                response_model = ""
...
def discover_logical_rules(
    tables: dict[str, list[Field]], ui_bindings: list[dict[str, str]]
    ...
    if override_node:
        for key_node, value_node in zip(override_node.keys, override_node.values):
            ...
            keywords = {
                item.arg: item.value for item in value_node.keywords if item.arg
            }
            required_node = keywords.get("required_fields") or (
                positional[0] if positional else None
            )
            unique_node = keywords.get("unique_fields") or (
                positional[1] if len(positional) > 1 else None
            )
            status_node = keywords.get("initial_status") or (
            ...
            overrides[(str(key[0]), str(key[1]))] = {
                ...
                "sensitive": isinstance(keywords.get("sensitive"), ast.Constant)
                and keywords["sensitive"].value is True,
                "immutable": isinstance(keywords.get("immutable"), ast.Constant)
                and keywords["immutable"].value is True,
    ...
    ui_resources = {
        (
            row["module"],
            canonical_by_compact.get((row["module"], row["entity"]), row["entity"]),
        )
            ...
            rows.append(
                {
                    ...
                    "persistence_decision": (
                        "typed_table"
                        if physical_target == f"{module}.{entity}"
                        and has_physical_target
                        else "typed_alias"
                        if has_physical_target
                    ...
                    "evidence": override.get(
                        "evidence", f"{path.relative_to(ROOT)}:74"
                    ),
                }
            )
    return rows
...
def discover_ui_bindings(tables: dict[str, list[Field]]) -> list[dict[str, str]]:
    ...
    contract_path = (
        ...
        / "src"
        / "config"
        / "entityFieldBindings.generated.json"
    )
    ...
    for path in sorted(pages.rglob("*.tsx")):
        ...
        for match in tag_pattern.finditer(text):
            ...
            if props["type"] == "list" and props["entity"] == props["module"]:
                continue
            entity = resource_aliases.get(
                f"{props['module']}:{props['entity']}", props["entity"]
            )
                ...
                bindings.append(
                    {
                        ...
                        "route": routes_by_file.get(
                            str(path.relative_to(ROOT)), "não localizada"
                        ),
                        "field": "não vinculado",
                        "binding": "sem entidade física/DTO correspondente",
                        "binding_status": "lacuna_entidade_sem_contrato",
                        "dto": "não localizado",
                        "endpoint": f"/{props['module']}/resources/{entity}",
                        "physical_storage": "não localizada",
                        "validation": "não localizada",
                        "contract_evidence": "contrato ausente",
            ...
            selected_fields = (
                contract["fields"]
                if props["type"] == "form"
                else [contract_fields[name] for name in contract["listFields"]]
            )
            for field in selected_fields:
                dto_binding = (
                    field["binding"]
                    if props["type"] == "form"
                    else f"response.payload.{field['name']}"
                )
                bindings.append(
                    {
                        "app": "all-in-one",
...
def discover_ui_actions(ui_bindings: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for surface_evidence, module, entity, surface, route in sorted(surfaces):
        ...
        if surface == "form":
                ...
                [
                    {
                        **common,
                        ...
                        "method": "local",
                        "endpoint": "navigate(-1)",
                        "request_contract": "não aplicável",
                        "backend_contract": "não aplicável",
                        "contract_status": "comprovado_local",
                        "frontend_permission_gate": "não aplicável",
                        "backend_enforcement": "não aplicável",
                        "audit": "não aplicável",
                        "states": ["idle"],
                        "test_evidence": "não localizado por superfície",
                        "evidence": source_evidence(source, "navigate(-1)"),
                    },
                    {
                        **common,
                        "action": "Salvar Registro",
                        "trigger": "submit",
                        "method": "POST (criação) / PATCH (edição)",
                        "endpoint": base,
                        ...
                        "contract_status": "compativel"
                        if save_contract_compatible
                        else "incompativel",
                        "frontend_permission_gate": "token com sub obrigatório",
                        "backend_enforcement": True,
                        "audit": "backend registra criação/edição aceita com ator e correlação",
                ...
                [
                    {
                        **common,
                        ...
                        "method": "GET",
                        "endpoint": base,
                        "request_contract": "query/limit",
                        "backend_contract": "lista protegida por actor_from_headers",
                        "contract_status": "parcial_com_fallback_local",
                        "frontend_permission_gate": False,
                        "backend_enforcement": True,
                        "audit": "leitura sensível não comprovada por superfície",
                        "states": ["loading", "erro", "sucesso"],
                        "test_evidence": "não localizado por superfície",
                    ...
                    },
                    {
                        **common,
                        ...
                        "trigger": "click",
                        "method": "local",
                        "endpoint": f"/{module}/{entity}-form",
                        "request_contract": "navegação",
                        "backend_contract": "não aplicável até salvar",
                        "contract_status": "comprovado_local",
                        "frontend_permission_gate": False,
                        "backend_enforcement": "não aplicável",
                        "audit": "não aplicável",
                    ...
                    },
                    {
                        **common,
                        ...
                        "trigger": "click",
                        "method": "local",
                        "endpoint": "modal em memória",
                        "request_contract": "registro já carregado",
                        "backend_contract": "não consulta GET de detalhe",
                        "contract_status": "comprovado_local_sem_refresh",
                        "frontend_permission_gate": False,
                        "backend_enforcement": False,
                        "audit": "leitura de detalhe não auditada no clique",
                    ...
                    },
                    {
                        **common,
                        ...
                        "method": "local",
                        "endpoint": f"/{module}/{entity}-form",
                        "request_contract": "registro em location.state",
                        "backend_contract": "salvamento posterior usa PATCH ResourcePatch",
                        "contract_status": "compativel_via_formulario"
                        if save_contract_compatible
                        else "parcial_edicao_incompativel",
                        "frontend_permission_gate": "token validado no salvamento",
                        "backend_enforcement": "no salvamento",
                    ...
                    },
                    {
                        **common,
                        ...
                        "method": "DELETE",
                        "endpoint": f"{base}/{{id}}",
                        "request_contract": "UUID e confirmação",
                        "backend_contract": "soft delete; imutáveis/sensíveis retornam 409",
                        "contract_status": "parcial_com_fallback_local",
                        "frontend_permission_gate": False,
                        "backend_enforcement": True,
                        "audit": "backend audita somente chamada remota aceita",
                        "states": ["running", "completed", "failed"],
                        "test_evidence": "não localizado por superfície",
                        "evidence": source_evidence(
                            source, "onClick={() => deleteRecord(item)}"
                        ),
                    },
                ]
            )
            journey = {
                ("marketplace", "orders"): (
                    "Autorizar pagamento sandbox",
                    "POST",
                    "/gateway/payments/sandbox/authorize",
                ),
                ("delivery", "delivery_requests"): (
                    "Executar jornada de entrega",
                    "POST x3",
                    "/delivery/resources/delivery_requests/{id}/actions/{assign|pickup|complete}",
                ),
                ("jobs", "job_postings"): (
                    "Enviar candidatura",
                    "POST x2",
                ...
                rows.append(
                    {
                        **common,
                        ...
                        "method": method,
                        "endpoint": endpoint,
                        "request_contract": "contrato específico implementado no SmartCRUD",
                        "backend_contract": "rotas específicas com actor/autorização",
                        "contract_status": "implementado_requer_e2e_vivo",
                        "frontend_permission_gate": "token e presença de dados; papel não verificado",
                        "backend_enforcement": True,
                        "audit": "backend conforme transições",
                        "states": ["running", "completed", "failed"],
                        "test_evidence": "testes de jornada existem, cobertura por tela requer confirmação",
                        "evidence": source_evidence(
                            source, "const runJourneyAction = async () =>"
                        ),
                    }
                )
    return rows
...
def discover_permission_enforcement(
    ...
    def test_evidence(entity: str, operation: str) -> list[str]:
        ...
        endpoint_pattern = re.compile(
            rf"\.{verb}\(\s*f?[\"']/resources/{re.escape(entity)}{suffix}"
        )
    ...
    generic_operations = (
        (
            ...
            "owner_or_operator",
            "store.create gera audit/event",
            "X-Idempotency-Key somente transacionais",
            '@app.post("/resources/{resource_type}"',
        ),
        (
            "list",
            "GET",
            "/resources/{resource_type}",
            "lista do actor; user_id de terceiro exige APPROVER_ROLES",
            "leitura não auditada genericamente",
            "não aplicável",
        ...
        (
            ...
            "owner_or_operator antes da exposição",
            "leitura não auditada genericamente",
            "não aplicável",
        ...
        (
            ...
            "/resources/{resource_type}/{resource_id}",
            "owner_or_operator",
            "store.update gera audit",
            "não exigida genericamente",
        ...
        (
            ...
            "DELETE",
            "/resources/{resource_type}/{resource_id}",
            "owner_or_operator; sensíveis/imutáveis bloqueados",
            "soft_delete gera audit",
            "não aplicável",
    ...
    for rule in logical_rules:
        ...
        for (
            ...
            idempotency,
            evidence_needle,
        ) in generic_operations:
            is_permissions = module == "permissions"
            horizontal_gap = (
                operation == "read"
                and not sensitive
                and not is_permissions
                and not generic_read_has_ownership
            )
            role_enforcement = (
            ...
            rows.append(
                {
                    ...
                    "operation": operation,
                    "method": method,
                    "endpoint": endpoint.replace("{resource_type}", entity),
                    "authentication": "actor_from_headers obrigatório",
                    "role_enforcement": role_enforcement,
                    "ownership_enforcement": ownership,
                    ...
                    "audit_enforcement": audit,
                    "idempotency": idempotency,
                    "enforcement_status": "lacuna_autorizacao_horizontal"
                    if horizontal_gap
                    else "parcial_estatico",
                    "test_evidence": test_evidence(entity, operation),
                    "evidence": source_evidence(runtime, evidence_needle),
                }
            )
    sensitive_by_resource = {
        (str(row["module"]), str(row["entity"])): bool(row["sensitive"])
        for row in logical_rules
    }
    for transition in transitions:
        module, entity, action = (
            str(transition["module"]),
            str(transition["entity"]),
        ...
        rows.append(
            {
                ...
                "operation": action,
                "method": "POST",
                "endpoint": f"/resources/{entity}/{{resource_id}}/actions/{action}",
                "authentication": "actor_from_headers obrigatório",
                ...
                "sensitive_resource": sensitive_by_resource.get(
                    (module, entity), False
                ),
    ...
    for path in sorted(test_paths):
        ...
        for node in ast.walk(tree):
            if not isinstance(
                ...
                continue
            assertions = [
                item for item in ast.walk(node) if isinstance(item, ast.Assert)
            ]
            calls = [item for item in ast.walk(node) if isinstance(item, ast.Call)]
            methods = sorted(
                {
                    call.func.attr.upper()
                    for call in calls
            ...
            decorators = [
                annotation_name(item.func if isinstance(item, ast.Call) else item)
            ...
            rows.append(
                {
                    ...
                    "status_code_assertions": status_assertions,
                    "http_methods": methods,
                    "endpoint_literals": endpoints,
                    "parametrized": any(
                        "parametrize" in decorator for decorator in decorators
                    ),
                    "decorators": decorators,
                    "execution_status": executed["status"]
                    if executed
                    else "não consta nos relatórios de execução",
...
def discover_memo_requirements(
    ...
    keyword_map = {
        "banc": ["database", "postgres", "mongodb", "sqlite", "redis"],
        "schema": ["schema", "migration"],
        "tabel": ["table", "store", "migration"],
        "cole": ["collection", "mongodb"],
        "camp": ["field", "payload", "catalog"],
        "relacion": ["relationship", "foreign", "erd"],
        "frontend": ["frontend", "ui", "stitch"],
        "binding": ["binding", "contract"],
        "sens": ["sensitive", "lgpd", "privacy"],
        "auditor": ["audit", "log", "trace"],
        "calcul": ["calculation", "precision", "decimal"],
        "unidad": ["unit", "conversion"],
        "tribut": ["tax", "fiscal"],
        "fisc": ["fiscal", "tax"],
        "formul": ["form", "smartcrud"],
        "bot": ["action", "button", "ui"],
        "permiss": ["permission", "authorization", "enforcement", "role"],
        "lacuna": ["gap", "coverage", "audit_delivery"],
        "teste": ["test", "pytest"],
        "document": ["document", "delivery", "markdown"],
        "evid": ["evidence", "traceability"],
        "evento": ["event", "outbox"],
        "api": ["api", "endpoint", "gateway"],
        "risco": ["risk", "security"],
        "persona": ["persona", "role"],
        "dominio": ["domain", "module"],
        "stitch": ["stitch"],
        "erd": ["erd"],
        "segur": ["security", "permission", "auth"],
        "e2e": ["e2e", "journey"],
        "respons": ["responsive", "mobile"],
        "acess": ["accessibility", "wcag"],
        "acao": ["action", "transition"],
        "estado": ["state", "status"],
    ...
    for number, line in enumerate(lines, 1):
        ...
        if section and line.startswith("## "):
            subsection = re.sub(
                r"[^a-z0-9]+",
                "_",
                unicodedata.normalize("NFKD", line[3:].casefold())
                .encode("ascii", "ignore")
                .decode("ascii"),
            ).strip("_")
            continue
        if not section or not line.startswith("- "):
            continue
        requirement = (
            re.sub(r"^- (?:\[[ xX]\]\s*)?", "", line).strip().rstrip(";").rstrip(".")
        )
        counters[section] += 1
        requirement_id = (
            f"{'CONCL' if section == 'conclusao' else 'CHECK'}-{counters[section]:03d}"
        )
        ...
        candidate_rows = [
            row for row, candidate_words in test_words if match_terms & candidate_words
        ][:20]
        candidates = [str(row["test_id"]) for row in candidate_rows]
        passed_candidates = [
            str(row["test_id"])
            for row in candidate_rows
            if row["execution_status"] == "aprovado"
        ]
        requirements.append(
            {
                ...
                "trace_status": "candidatos_localizados"
                if candidates
                else "sem_teste_localizado",
                "execution_status": "candidatos_aprovados"
                if passed_candidates
                else "sem_candidato_aprovado",
...
def write_csv(
    path: Path, columns: list[str], rows: Iterable[dict[str, object]]
) -> None:
...
def build_delivery() -> None:
    ...
    sensitive = [
        field for field in fields if field.lgpd != "não classificado automaticamente"
    ]
    counts = {
        ...
        "endpoints_with_response_model": sum(
            bool(row["response_model"]) for row in endpoints
        ),
        "api_model_fields": sum(
            len(parameter["model_fields"])
            for row in endpoints
            for parameter in row["parameters"]
        ),
        "ui_candidates": len(ui_bindings),
        "ui_bindings_probable": sum(
            row["binding_status"] == "comprovado_por_contrato_versionado"
            for row in ui_bindings
        ),
        "ui_bindings_unproven": sum(
            row["binding_status"] != "comprovado_por_contrato_versionado"
            for row in ui_bindings
        ),
        "ui_surfaces": len({row["evidence"] for row in ui_bindings}),
        "ui_forms": len(
            {row["evidence"] for row in ui_bindings if row["surface"] == "form"}
        ),
        "ui_actions": len(ui_actions),
        "ui_actions_incompatible": sum(
            row["contract_status"] == "incompativel" for row in ui_actions
        ),
        "ui_actions_without_frontend_permission_gate": sum(
            row["frontend_permission_gate"] is False for row in ui_actions
        ),
        "permission_operations": len(permission_enforcement),
        "permission_horizontal_read_gaps": sum(
            row["enforcement_status"] == "lacuna_autorizacao_horizontal"
            for row in permission_enforcement
        ),
        "permission_operations_with_test_candidates": sum(
            bool(row["test_evidence"]) for row in permission_enforcement
        ),
        ...
        "test_functions_in_execution_reports": sum(
            row["execution_status"] != "não consta nos relatórios de execução"
            for row in test_catalog
        ),
        "test_functions_passed": sum(
            row["execution_status"] == "aprovado" for row in test_catalog
        ),
        "memo_requirements_traced": len(memo_requirements),
        "memo_requirements_without_test_candidates": sum(
            not row["test_candidates"] for row in memo_requirements
        ),
        "memo_requirements_with_passed_candidates": sum(
            bool(row["passed_test_candidates"]) for row in memo_requirements
        ),
        "logical_entities": len(logical_rules),
        "logical_without_physical_table": sum(
            not row["has_physical_table"] for row in logical_rules
        ),
        "logical_without_ui_surface": sum(
            not row["has_ui_surface"] for row in logical_rules
        ),
    ...
    product_units_tax_model = json.loads(
        PRODUCT_UNITS_TAX_MODEL.read_text(encoding="utf-8")
    )
    field_classification_policy = json.loads(
        FIELD_CLASSIFICATION_POLICY.read_text(encoding="utf-8")
    )
    audit_policy = json.loads(AUDIT_TRACEABILITY_POLICY.read_text(encoding="utf-8"))
    stitch_template_coordinate = json.loads(
        STITCH_TEMPLATE_COORDINATE.read_text(encoding="utf-8")
    )
    counts["dynamic_form_entities_proposed"] = len(dynamic_form_model["entities"])
    counts["measurement_entities_proposed"] = len(
        product_units_tax_model["measurement_entities"]
    )
    ...
    audit_tables = {
        ...
        or any(token in key for token in ("audit", "_logs", "events"))
    }
            ...
            audit_coverage.append(
                {
                    ...
                    "evidence": [
                        f"docs/data-audit/databases/postgresql/tables/{table}.md"
    ...
    counts["audit_requirements_covered"] = sum(
        bool(row["covered"]) for row in audit_coverage
    )

    field_rows = [asdict(field) for field in fields]
    write_csv(
        ARTIFACTS / "dicionario_de_dados.csv", list(Field.__annotations__), field_rows
    )
    write_json(
        ARTIFACTS / "dicionario_de_dados.json",
        {"version": 1, "counts": counts, "fields": field_rows},
    )
    mongodb_csv_rows = [
        {
            **row,
            "enum": ";".join(row["enum"]),
            "indexes": json.dumps(row["indexes"], ensure_ascii=False),
        }
    ...
    write_json(
        ARTIFACTS / "catalogo_mongodb.json",
        {"version": 1, "counts": counts, "fields": mongodb_fields},
    )
    write_csv(
        ARTIFACTS / "catalogo_sqlite.csv", list(Field.__annotations__), sqlite_fields
    )
    write_json(
        ARTIFACTS / "catalogo_sqlite.json",
        {"version": 1, "counts": counts, "fields": sqlite_fields},
    )
    logical_csv_rows = [
        {
            **row,
            "required_fields": ";".join(row["required_fields"]),
            "unique_fields": ";".join(row["unique_fields"]),
            "monetary_fields": ";".join(row["monetary_fields"]),
        }
        for row in logical_rules
    ]
    write_csv(
    ...
    write_json(
        ARTIFACTS / "catalogo_logico.json",
        {"version": 1, "counts": counts, "entities": logical_rules},
    )
    event_csv_rows = [
        {
            **row,
    ...
    write_json(
        ARTIFACTS / "catalogo_eventos.json",
        {"version": 1, "counts": counts, "events": transitions},
    )
    api_csv_rows = [
        {
            **row,
    ...
    write_json(
        ARTIFACTS / "catalogo_apis.json",
        {"version": 1, "counts": counts, "endpoints": endpoints},
    )
    write_json(
        ARTIFACTS / "matriz_acao_ui_backend.json",
        {
            "version": 1,
            "status": "auditoria_estatica",
            "counts": counts,
            "actions": ui_actions,
        },
    )
    ...
    write_json(
        ARTIFACTS / "matriz_enforcement_permissao.json",
        {
            "version": 1,
            "status": "auditoria_estatica",
            "counts": counts,
            "operations": permission_enforcement,
        },
    )
    write_csv(
        ...
        (
            {
                **row,
                "role_enforcement": ";".join(row["role_enforcement"])
                if isinstance(row["role_enforcement"], list)
                else row["role_enforcement"],
    ...
    write_json(
        ARTIFACTS / "catalogo_testes.json",
        {
            "version": 1,
            "status": "inventario_estatico",
            "counts": counts,
            "tests": test_catalog,
        },
    )
    write_csv(
        ...
        (
            {
                **row,
                "http_methods": ";".join(row["http_methods"]),
                "endpoint_literals": ";".join(row["endpoint_literals"]),
                "decorators": ";".join(row["decorators"]),
            }
            for row in test_catalog
        ),
    )
    write_json(
        ARTIFACTS / "matriz_requisito_teste.json",
        {
            "version": 1,
            "status": "rastreabilidade_estatica",
            "counts": counts,
    ...
    write_csv(
        ...
        (
            {
                **row,
                "test_candidates": ";".join(row["test_candidates"]),
                "passed_test_candidates": ";".join(row["passed_test_candidates"]),
            }
            for row in memo_requirements
        ),
    )
    for artifact_name, rows in (
        ...
        write_json(
            ARTIFACTS / f"{artifact_name}.json",
            {
                "version": 1,
                "status": "inventario_estatico",
                "counts": counts,
    ...
    write_json(
        ARTIFACTS / "politica_classificacao_campos.json", field_classification_policy
    )
    write_json(
        ARTIFACTS / "coordenada_projetos_stitch.json", stitch_template_coordinate
    )
    write_json(
        ARTIFACTS / "cobertura_auditoria.json",
        {
            ...
            "policy": audit_policy,
            "counts": counts,
            "candidate_tables": sorted(audit_tables),
        ...
        actions_by_surface[str(action["surface_evidence"])].append(
            {
                ...
                if key
                not in {"surface_evidence", "surface", "route", "module", "entity"}
            }
        )
    required_states = [
        "loading",
    ...
    for row in ui_bindings:
        ...
        surface = surfaces.setdefault(
            evidence,
            {
                ...
                "fields": [],
                "primary_action": "Salvar registro"
                if row["surface"] == "form"
                else "Abrir detalhes",
    ...
    coordinate_rows = [
        {
            **surface,
            "fields": ";".join(
                f"{item['name']}:{item['binding']}" for item in surface["fields"]
            ),
    ...
    write_json(
        ARTIFACTS / "coordenadas_stitch.json",
        {"version": 1, "counts": counts, "coordinates": list(surfaces.values())},
    )
    write_csv(
        ...
        or [
            {
                ...
                "title": "",
                "route": "",
                "field": "não localizado",
                "binding": "lacuna",
                "binding_status": "lacuna",
                "dto": "",
                "endpoint": "",
                "physical_storage": "",
                "validation": "",
    ...
    write_csv(
        ...
        [
            {
                "risk": "binding UI sem comprovação backend",
                "priority": "P1",
                "control": "matriz API x DTO x campo",
                "status": "pendente",
                "evidence": "docs/data-audit/artifacts/matriz_formulario_campo.csv",
            },
            {
                "risk": "classificação LGPD heurística",
                "priority": "P0",
                "control": "revisão por proprietário do domínio",
                "status": "pendente",
                "evidence": "docs/data-audit/artifacts/dicionario_de_dados.csv",
            },
        ],
    )
    ...
    for key in sorted(tables):
        ...
        for field in tables[key]:
            ...
            erd.append(
                f"    {field.physical_type.replace(' ', '_')} {field.physical_name}{marker}"
            )
        erd.append("  }")
    for field in relations:
        target = field.reference.rsplit(".", 1)[0].replace(".", "_")
        erd.append(
            f'  {field.schema}_{field.table} }}o--|| {target} : "{field.physical_name}"'
        )
    ...
    gaps = [
        {
            ...
            "title": "Persistências não PostgreSQL exigem catálogo e validação operacional",
            ...
            "evidence": [
                "infra/docker/docker-compose.yml:69",
                "docs/data-audit/artifacts/catalogo_mongodb.json",
                "docs/data-audit/artifacts/catalogo_sqlite.json",
            ],
            "impact": "Retenção, recuperação, ownership e divergências entre configuração e ambiente não são comprovados.",
            "risk": "Perda, exposição ou inconsistência de dados fora do PostgreSQL.",
            "proposal": "Catalogar chaves, objetos, buckets e políticas; adicionar sondagens não destrutivas e testes por tecnologia.",
            "dependencies": ["credenciais dos ambientes", "serviços acessíveis"],
            "affected_files": [
                "infra/docker/docker-compose.yml",
                "docs/data-audit/databases",
            ],
            "migration": "avaliar por tecnologia",
            "backend": "inventariar clientes e contratos",
            "frontend": "inventariar chaves de storage",
            "tests": "testes de contrato e sondagem runtime",
            "documentation": "03_CATALOGO_DE_BANCOS.md",
            "acceptance": "MongoDB, SQLite, Redis, object storage e storage de navegador possuem catálogo, retenção, ownership, restore e testes aprovados.",
            "status": "parcial",
            "owner_suggestion": "plataforma e dados",
            "dimensions": ["bancos", "tabelas_colecoes", "campos"],
        },
        {
            "id": "AUD-P0-001",
            "priority": "P0",
            "module": "compliance",
            "title": "Classificação LGPD exige revisão humana por campo",
            "description": "A classificação atual é heurística e ainda não foi aprovada pelos proprietários dos domínios.",
            "evidence": [
                "docs/data-audit/artifacts/politica_classificacao_campos.json",
                "docs/data-audit/artifacts/dicionario_de_dados.csv",
            ],
            "impact": "Mascaramento, retenção e acesso podem não refletir finalidade e base legal reais.",
            "risk": "Tratamento indevido de dados pessoais e sensíveis.",
            "proposal": "Executar revisão campo a campo com DPO e responsáveis dos domínios, registrando decisão e vigência.",
            "dependencies": ["DPO", "proprietários de domínio"],
            "affected_files": ["config/data_audit/field_classification_policy.json"],
            "migration": "não aplicável até decisão",
            "backend": "aplicar classificação aprovada",
            "frontend": "aplicar mascaramento aprovado",
            "tests": "testar acesso e mascaramento por classe",
            "documentation": "11_PERMISSOES_E_SEGURANCA.md",
            "acceptance": "Todos os campos possuem classificação, retenção, criptografia e mascaramento aprovados pelo proprietário do domínio.",
            "status": "pendente_aprovacao",
            "owner_suggestion": "DPO e compliance",
            "dimensions": ["campos_sensiveis"],
        },
        {
            "id": "AUD-P1-002",
            "priority": "P1",
            "module": "frontend e APIs",
            "title": "Bindings frontend-backend não estão integralmente comprovados",
            "description": f"As 120 entidades persistentes possuem contrato UI versionado gerado das migrations e regras de domínio; o SmartCRUD renderiza e envia somente campos vinculados. Há {counts['ui_bindings_unproven']} superfícies/campos legados sem entidade física/DTO correspondente, além da execução E2E autenticada ainda pendente.",
            "evidence": [
                "apps/all-in-one/src/config/entityFieldBindings.generated.json",
                "apps/all-in-one/src/components/SmartCRUD.tsx",
                "docs/data-audit/artifacts/matriz_formulario_campo.csv",
            ],
            "impact": "Bindings persistentes estão comprovados, mas superfícies legadas ficam bloqueadas com segurança e integrações específicas ainda podem falhar em runtime.",
            "risk": "Funcionalidade indisponível nas superfícies sem contrato ou divergência operacional não detectada sem E2E.",
            "proposal": "Decidir entidade/DTO das superfícies legadas e executar matriz E2E autenticada em homologação.",
            "dependencies": [
                "decisão de domínio",
                "ambiente homologado",
                "credenciais de teste",
            ],
            "affected_files": ["apps/all-in-one/src", "modules"],
            "migration": "avaliar para superfícies legadas",
            "backend": "DTO genérico versionado com payload validado por regra de domínio",
            "frontend": "contratos específicos renderizados dinamicamente; superfícies sem contrato bloqueadas",
            "tests": "build aprovado; contrato estático e E2E por entidade necessários",
            "documentation": "09_FORMULARIOS_FRONTEND.md",
            "acceptance": "Cada campo UI aponta para DTO, endpoint, validação, coluna e E2E aprovado.",
            "status": "implementacao_parcial",
            "owner_suggestion": "frontend e backend",
            "dimensions": ["bindings_frontend", "formularios"],
        },
        {
            "id": "AUD-P1-003",
            "priority": "P1",
            "module": "eventos",
            "title": "Eventos não possuem catálogo integral de payload versionado",
            "description": "Os produtores persistentes usam envelope v1 compartilhado com sanitização, idempotência, correlação, causação, retenção, replay e compatibilidade; consumidores downstream específicos ainda não estão demonstrados para todos os eventos.",
            "evidence": [
                "modules/shared/event_contract.py",
                "tests/test_event_contract.py",
                "docs/data-audit/artifacts/catalogo_eventos.json",
                "docs/data-audit/artifacts/matriz_evento_campo.csv",
            ],
            "impact": "Sem consumidores declarados por evento, mudanças ainda podem quebrar integrações downstream.",
            "risk": "Entrega assíncrona sem ownership integral do consumidor.",
            "proposal": "Registrar e testar consumidores reais por evento, mantendo o envelope compartilhado como gate de produtor.",
            "dependencies": ["consumidores downstream", "mensageria homologada"],
            "affected_files": [
                "modules/shared/event_contract.py",
                "modules/shared/outbox_dispatcher.py",
                "contracts",
            ],
            "migration": "não aplicável",
            "backend": "contrato de publicação implementado; conectar consumidores reais",
            "frontend": "não aplicável salvo atualizações em tempo real",
            "tests": "envelope, sigilo, idempotência, correlação, replay e compatibilidade aprovados; E2E de consumidor pendente",
            "documentation": "12_APIS_EVENTOS_E_INTEGRACOES.md",
            "acceptance": "Cada evento possui produtor, consumidor, schema, dados proibidos, idempotência, correlação, retenção e compatibilidade testados.",
            "status": "implementacao_parcial",
            "owner_suggestion": "arquitetura de integração",
            "dimensions": ["campos", "relacionamentos"],
        },
        {
            "id": "AUD-P1-004",
            "priority": "P1",
            "module": "formulários dinâmicos",
            "title": "Construtor de formulários dinâmicos possui persistência, mas ainda não está operacional",
            "description": "A migration 028, o serviço dynamic_forms e o builder web implementam catálogo, bindings, versões, blueprint, prévia responsiva, cálculos allowlist, homologação segregada, publicação com MFA, submissões idempotentes, eventos e cobrança; a homologação PostgreSQL operacional permanece pendente.",
            "evidence": [
                "database/postgres/migrations/028_dynamic_forms_governance.sql",
                "modules/dynamic_forms/main.py",
                "modules/shared/dynamic_forms.py",
                "modules/shared/dynamic_forms_postgres_store.py",
                "apps/all-in-one/src/pages/DynamicFormBuilder.tsx",
                "tests/test_dynamic_forms_api.py",
                "tests/test_dynamic_forms_domain.py",
            ],
            "impact": "Sem validação em PostgreSQL vivo, o produto ainda não pode ser homologado ponta a ponta.",
            "risk": "Comportamento SQL ainda não comprovado em ambiente real.",
            "proposal": "Executar a migration e os fluxos completos em PostgreSQL homologado.",
            "dependencies": ["PostgreSQL de homologação"],
            "affected_files": [
                "apps/all-in-one/src",
                "modules/dynamic_forms",
                "database/postgres/migrations/028_dynamic_forms_governance.sql",
            ],
            "migration": "implementada e reversível",
            "backend": "APIs, RBAC/ABAC, homologação, publicação, submissão e eventos implementados",
            "frontend": "builder allowlist, preview responsivo e fluxo de publicação implementados",
            "tests": "domínio, segurança HTTP, tenant, MFA, idempotência e QA visual aprovados; PostgreSQL vivo pendente",
            "documentation": "10_FORMULARIOS_DINAMICOS.md",
            "acceptance": "Metadados, API, homologação, segurança, publicação, cobrança, auditoria e testes estão implementados.",
            "status": "implementacao_parcial",
            "owner_suggestion": "produto, arquitetura e engenharia",
            "dimensions": [
                "tabelas_colecoes",
                "campos",
                "formularios",
        ...
        },
        {
            "id": "AUD-P1-005",
            "priority": "P1",
            "module": "catálogo e fiscal",
            "title": "Regras fiscais, unidades e conversões carecem de implementação completa",
            "description": "Migration reversível, rollback, cálculo Decimal, contratos HTTP, console web e testes de integração foram implementados; faltam integração PostgreSQL viva e homologação por especialista fiscal.",
            "evidence": [
                "docs/data-audit/artifacts/modelo_unidades_tributacao.json",
                "database/postgres/migrations/025_units_tax_governance.sql",
                "database/postgres/rollbacks/025_units_tax_governance.down.sql",
                "modules/shared/units_tax.py",
                "modules/stock/main.py",
                "modules/erp/main.py",
                "apps/all-in-one/src/pages/UnitsTaxGovernance.tsx",
                "tests/test_units_tax_governance.py",
            ],
            "impact": "Sem os gates externos restantes, compra, estoque, venda, custo e tributação ainda podem divergir em operação.",
            "risk": "Cálculo financeiro ou fiscal não homologado em ambiente operacional.",
            "proposal": "Executar integração PostgreSQL viva e homologar cenários com especialista fiscal.",
            "dependencies": ["especialista fiscal", "ambiente PostgreSQL"],
            "affected_files": ["database/postgres", "modules/stock", "modules/erp"],
            "migration": "implementada com rollback; backfill explícito sem inferência",
            "backend": "Decimal, vigência, aprovação, precisão, arredondamento e contratos HTTP implementados",
            "frontend": "console responsivo de unidades e cálculo fiscal implementado",
            "tests": "unitários e contratos HTTP aprovados; PostgreSQL vivo e homologação fiscal pendentes",
            "documentation": "06_UNIDADES_E_CONVERSOES.md e 07_TRIBUTACAO.md",
            "acceptance": "Unidades, conversões, perfis fiscais, vigência e cálculos possuem migrations, backend, frontend, integração e homologação aprovados.",
            "status": "implementacao_parcial",
            "owner_suggestion": "catálogo, estoque, ERP e fiscal",
            "dimensions": [
                "tabelas_colecoes",
                "campos",
                "calculos",
        ...
        },
        {
            "id": "AUD-P1-006",
            "priority": "P1",
            "module": "arquitetura modular",
            "title": "Entidades lógicas sem decisão de persistência homônima",
            "description": "As migrations 026 e 027 materializaram os 57 destinos antes ausentes, cinco aliases físicos foram reconhecidos e todas as 120 entidades lógicas agora possuem persistência tipada e superfície UI canônica.",
            "evidence": [
                "docs/data-audit/artifacts/catalogo_logico.json",
                "database/postgres/migrations/026_complete_typed_store_relations.sql",
                "database/postgres/migrations/027_complete_legacy_adapter_relations.sql",
                "modules/shared/generic_postgres_resource.py",
                "tests/test_typed_store_persistence_contract.py",
            ],
            "impact": "Contrato de persistência lógica e física alinhado.",
            "risk": "Controlado por teste de completude e rollback.",
            "proposal": "Manter o gate em toda inclusão de entidade ou store.",
            "dependencies": [],
            "affected_files": [
                "modules/shared/*_postgres_store.py",
                "database/postgres/migrations",
            ],
            "migration": "57 relações implementadas em duas migrations reversíveis",
            "backend": "120 de 120 entidades com destino físico tipado",
            "frontend": "120 de 120 entidades com superfície canônica",
            "tests": "contrato de destino tipado, aliases e reversibilidade aprovados",
            "documentation": "02_MAPA_DE_DOMINIOS.md",
            "acceptance": "Cada entidade possui decisão explícita de persistência, ownership e coordenada UI, ou justificativa versionada de ausência.",
            "status": "resolvido",
            "owner_suggestion": "arquitetura e responsáveis de domínio",
            "dimensions": [
                "tabelas_colecoes",
                "campos",
                "relacionamentos",
        ...
        },
        {
            "id": "AUD-P1-007",
            "priority": "P1",
            "module": "auditoria e segurança",
            "title": "Contrato unificado de auditoria ainda exige homologação operacional por módulo",
            "description": "A migration 029 e o writer compartilhado cobrem estruturalmente os 35 requisitos, são usados pela base e pelos sete stores PostgreSQL especializados e capturam contexto HTTP governado. Leituras sensíveis registram finalidade, exportação, impressão e compartilhamento. A aplicação da migration em PostgreSQL vivo e a homologação positiva e negativa por operação sensível ainda não estão comprovadas.",
            "evidence": [
                "database/postgres/migrations/029_unified_immutable_audit.sql",
                "modules/shared/audit_contract.py",
                "modules/shared/store.py",
                "modules/shared/postgres_store.py",
                "modules/shared/runtime.py",
                "tests/test_audit_contract.py",
                "tests/test_permissions_enforcement_matrix.py",
                "docs/data-audit/artifacts/cobertura_auditoria.json",
            ],
            "impact": "Sem PostgreSQL vivo e matriz operacional, a integridade e o acesso da trilha não estão homologados ponta a ponta.",
            "risk": "Não repúdio e comprovação LGPD ainda dependem de validação operacional.",
            "proposal": "Aplicar a migration em homologação e executar matriz positiva e negativa de mutação, leitura, exportação, impressão e compartilhamento por módulo sensível.",
            "dependencies": [
                "PostgreSQL de homologação",
                "segurança",
                "compliance",
                "proprietários de domínio",
            ],
            "affected_files": [
                "database/postgres/migrations/029_unified_immutable_audit.sql",
            ...
            "migration": "029 implementada e reversível; aplicação em PostgreSQL vivo pendente",
            "backend": "writer único, stores especializados, contexto HTTP e leitura sensível implementados",
            "frontend": "enviar motivo quando obrigatório e nunca registrar segredo",
            "tests": "contrato, integridade, minimização, contexto HTTP e leitura sensível aprovados; PostgreSQL vivo por operação pendente",
            "documentation": "08_AUDITORIA_E_LOGS.md",
            "acceptance": "Os 35 requisitos de auditoria têm implementação ou decisão justificada por operação, com integridade, retenção e testes aprovados.",
            "status": "implementacao_parcial",
            "owner_suggestion": "segurança, plataforma e compliance",
            "dimensions": ["auditoria", "campos_sensiveis", "permissoes_backend"],
        },
        {
            "id": "AUD-P1-008",
            "priority": "P1",
            "module": "frontend e runtime compartilhado",
            "title": "Salvar Registro é incompatível com o contrato CRUD do backend",
            ...
            "evidence": [
                ...
                "docs/data-audit/artifacts/matriz_acao_ui_backend.json",
            ],
            "impact": "Criação e edição autenticadas podem falhar se o contrato regredir.",
            "risk": "Botão principal não conclui a operação e o fallback local mascara a incompatibilidade.",
            "proposal": "Manter adapter frontend tipado por entidade, POST {user_id,payload}, PATCH {payload}, idempotência e testes HTTP/E2E.",
            "dependencies": [
                "bindings de campo",
                "actor autenticado",
            ...
            "migration": "não aplicável",
            "backend": "manter contratos explícitos e documentar métodos",
            "frontend": "manter envelope, método, idempotência e gates de permissão",
            "tests": "contrato HTTP e E2E para criação/edição das 129 superfícies",
            "documentation": "09_FORMULARIOS_FRONTEND.md",
            "acceptance": "Cada formulário cria e edita via contrato backend compatível, com permissão, estados, auditoria e teste aprovado.",
            "status": "resolvido",
            "owner_suggestion": "frontend e plataforma backend",
            "dimensions": [
                "bindings_frontend",
                "formularios",
                "acoes_ui",
        ...
        },
        {
            "id": "AUD-P0-009",
            "priority": "P0",
            "module": "runtime compartilhado",
            "title": "Leitura por ID não aplica ownership para recursos não sensíveis",
            "description": "A rota genérica GET por resource_id autentica o ator, mas _expose só restringe leitura de terceiro quando a regra é sensível. Das 61 entidades não sensíveis, 56 fora do módulo permissions não recebem verificação de proprietário, tenant ou papel nessa rota.",
            "evidence": [
                source_evidence(
                    "modules/shared/runtime.py",
                    '@app.get("/resources/{resource_type}/{resource_id}"',
            ...
            "risk": "IDOR e quebra de isolamento horizontal/multitenant.",
            "proposal": "Aplicar autorização owner/operator ou política ABAC/tenant antes de _expose, negar por padrão e criar testes negativos para todas as classes de recurso.",
            "dependencies": [
                "contrato de ownership por entidade",
                "tenant/business context",
            ],
            "affected_files": ["modules/shared/runtime.py", "tests"],
            "migration": "não aplicável",
            "backend": "enforcement deny-by-default em get_resource",
            "frontend": "não confiar em ocultação de links ou UUIDs",
            "tests": "teste negativo de leitura cruzada para as 56 entidades e teste positivo autorizado",
            "documentation": "11_PERMISSOES_E_SEGURANCA.md",
            "acceptance": "Toda leitura por ID valida owner, tenant ou papel/atributo explicitamente autorizado e possui teste negativo/positivo por política.",
            "status": "incompativel_seguranca",
            "owner_suggestion": "segurança e plataforma backend",
            "dimensions": ["permissoes_backend", "auditoria"],
        },
    ]
    ...
    gap_counts = {
        priority: sum(gap["priority"] == priority for gap in gaps)
        for priority in ("P0", "P1", "P2", "P3", "P4")
    }
    write_json(
        ARTIFACTS / "relatorio_divergencias.json",
        {
            "version": 2,
            "status": "em_execucao",
            "counts": {"total": len(gaps), **gap_counts},
            "required_fields": [
                "id",
                "title",
                "module",
    ...
    coverage_values = {
        ...
        "tabelas_colecoes": 85,
        "campos": 75,
        "relacionamentos": coverage_ratio(
            sum(bool(field.evidence and field.reference) for field in relations),
            len(relations),
        ),
        "bindings_frontend": coverage_ratio(
            counts["ui_bindings_probable"], counts["ui_candidates"]
        ),
        "campos_sensiveis": 0,
        "auditoria": coverage_ratio(
            counts["audit_requirements_covered"], counts["audit_requirements"]
        ),
        "calculos": coverage_ratio(
            sum(bool(units_gate[name]) for name in calculations_gate_names),
            len(calculations_gate_names),
        ),
        "unidades": coverage_ratio(
            sum(bool(units_gate[name]) for name in units_gate_names),
            len(units_gate_names),
        ),
        "regras_fiscais": coverage_ratio(
            sum(bool(units_gate[name]) for name in fiscal_gate_names),
            len(fiscal_gate_names),
        ),
        "formularios": coverage_ratio(counts["ui_surfaces"], counts["ui_surfaces"]),
        "acoes_ui": coverage_ratio(
            counts["ui_actions"] - counts["ui_actions_incompatible"],
            counts["ui_actions"],
        ),
        "permissoes_backend": coverage_ratio(
            counts["permission_operations"] - counts["permission_horizontal_read_gaps"],
            counts["permission_operations"],
        ),
        "lacunas_com_backlog": 100,
    }
    evidence_by_dimension = {
        "bancos": ["docs/data-audit/03_CATALOGO_DE_BANCOS.md"],
        "schemas": ["docs/data-audit/artifacts/dicionario_de_dados.json"],
        "tabelas_colecoes": [
            "docs/data-audit/artifacts/dicionario_de_dados.json",
            "docs/data-audit/artifacts/catalogo_mongodb.json",
            "docs/data-audit/artifacts/catalogo_sqlite.json",
        ],
        "campos": ["docs/data-audit/artifacts/dicionario_de_dados.json"],
        "relacionamentos": ["docs/data-audit/artifacts/erd.mmd"],
        "bindings_frontend": ["docs/data-audit/artifacts/matriz_formulario_campo.csv"],
        "campos_sensiveis": [
            "docs/data-audit/artifacts/politica_classificacao_campos.json"
        ],
        "auditoria": ["docs/data-audit/artifacts/cobertura_auditoria.json"],
        "calculos": ["docs/data-audit/artifacts/modelo_unidades_tributacao.json"],
        "unidades": ["docs/data-audit/artifacts/modelo_unidades_tributacao.json"],
        "regras_fiscais": ["docs/data-audit/artifacts/modelo_unidades_tributacao.json"],
        "formularios": ["docs/data-audit/artifacts/coordenadas_stitch.json"],
        "acoes_ui": ["docs/data-audit/artifacts/coordenadas_stitch.json"],
        "permissoes_backend": [
            "docs/data-audit/artifacts/matriz_permissao_acao.csv",
            "docs/data-audit/artifacts/matriz_enforcement_permissao.json",
        ],
        "lacunas_com_backlog": [
            "docs/data-audit/artifacts/relatorio_divergencias.json"
        ],
    }
    dimensions = {}
    ...
    for key, table_fields in sorted(tables.items()):
        ...
        (DATABASE / "tables" / f"{schema}.{table}.md").write_text(
            f"# `{key}`\n\n{table_body}\n", encoding="utf-8"
        )
...
Nenhuma rota proposta é apresentada como existente. EVIDÊNCIAS: `apps/all-in-one/src/App.tsx`, `artifacts/coordenadas_stitch.json`, `docs/MEMORANDO_MESTRE_GEMINI_VARREDURA_DADOS_FORMULARIOS_ALL_IN_ONE.md:2545`.""",
    ...
    write_markdown(
        ...
        f"A matriz cobre os {len(memo_requirements)} requisitos explícitos das seções de conclusão e checklist. Candidatos não são promovidos a prova.\n\n{trace_rows}\n\nEVIDÊNCIAS: `artifacts/matriz_requisito_teste.json` e `artifacts/catalogo_testes.json`.",
    )
    acceptance_rows = markdown_table(
        ...
        (
            [
                ...
                else "; ".join(item["lacunas"]) or "cobertura parcial",
            ]
