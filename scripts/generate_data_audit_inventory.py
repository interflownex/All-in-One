...
def build_delivery() -> None:
    ...
    gaps = [
        ...
        },
        {
            ...
            "description": "A migration 028 materializa metadados, catálogo, bindings, versões, blocos, campos, cálculos, validações, permissões, homologação, publicação, submissões e eventos faturáveis; APIs, builder web e homologação operacional permanecem pendentes.", "evidence": ["database/postgres/migrations/028_dynamic_forms_governance.sql", "database/postgres/rollbacks/028_dynamic_forms_governance.down.sql", "tests/test_dynamic_forms_migration.py", "docs/data-audit/artifacts/formulario_dinamico_modelo.json"], "impact": "Sem APIs e UI, o produto pago ainda não pode ser usado de ponta a ponta.", "risk": "Integrações ad hoc podem contornar a governança já materializada.", "proposal": "Implementar store e APIs sobre a migration, builder allowlist, prévia, homologação, publicação e cobrança.", "dependencies": ["autorização RBAC/ABAC", "API Hub"], "affected_files": ["database/postgres/migrations/028_dynamic_forms_governance.sql", "modules", "apps/all-in-one/src"], "migration": "implementada e reversível", "backend": "implementar APIs, regras e homologação", "frontend": "implementar builder, preview e publicação", "tests": "migration aprovada; segurança, ciclos, versionamento, tenant e cobrança runtime pendentes", "documentation": "10_FORMULARIOS_DINAMICOS.md", "acceptance": "Metadados, API, homologação, segurança, publicação, cobrança, auditoria e testes estão implementados.", "status": "implementacao_parcial", "owner_suggestion": "produto, arquitetura e engenharia", "dimensions": ["tabelas_colecoes", "campos", "formularios", "acoes_ui", "permissoes_backend", "auditoria", "calculos"]
        },
        {
