...
def build_delivery() -> None:
    ...
    gaps = [
        ...
        {
            "id": "AUD-P1-004", "priority": "P1", "module": "formulários dinâmicos", "title": "Construtor de formulários dinâmicos possui persistência, mas ainda não está operacional",
            "description": "A migration 028, o serviço dynamic_forms e o builder web implementam catálogo, bindings, versões, blueprint, prévia responsiva, cálculos allowlist, homologação segregada, publicação com MFA, submissões idempotentes, eventos e cobrança; a homologação PostgreSQL operacional permanece pendente.", "evidence": ["database/postgres/migrations/028_dynamic_forms_governance.sql", "modules/dynamic_forms/main.py", "modules/shared/dynamic_forms.py", "modules/shared/dynamic_forms_postgres_store.py", "apps/all-in-one/src/pages/DynamicFormBuilder.tsx", "tests/test_dynamic_forms_api.py", "tests/test_dynamic_forms_domain.py"], "impact": "Sem validação em PostgreSQL vivo, o produto ainda não pode ser homologado ponta a ponta.", "risk": "Comportamento SQL ainda não comprovado em ambiente real.", "proposal": "Executar a migration e os fluxos completos em PostgreSQL homologado.", "dependencies": ["PostgreSQL de homologação"], "affected_files": ["apps/all-in-one/src", "modules/dynamic_forms", "database/postgres/migrations/028_dynamic_forms_governance.sql"], "migration": "implementada e reversível", "backend": "APIs, RBAC/ABAC, homologação, publicação, submissão e eventos implementados", "frontend": "builder allowlist, preview responsivo e fluxo de publicação implementados", "tests": "domínio, segurança HTTP, tenant, MFA, idempotência e QA visual aprovados; PostgreSQL vivo pendente", "documentation": "10_FORMULARIOS_DINAMICOS.md", "acceptance": "Metadados, API, homologação, segurança, publicação, cobrança, auditoria e testes estão implementados.", "status": "implementacao_parcial", "owner_suggestion": "produto, arquitetura e engenharia", "dimensions": ["tabelas_colecoes", "campos", "formularios", "acoes_ui", "permissoes_backend", "auditoria", "calculos"]
        },
        {
        ...
        },
        {
            ...
            "description": "As migrations 026 e 027 materializaram os 57 destinos antes ausentes, cinco aliases físicos foram reconhecidos e todas as 120 entidades lógicas agora possuem persistência tipada e superfície UI canônica.", "evidence": ["docs/data-audit/artifacts/catalogo_logico.json", "database/postgres/migrations/026_complete_typed_store_relations.sql", "database/postgres/migrations/027_complete_legacy_adapter_relations.sql", "modules/shared/generic_postgres_resource.py", "tests/test_typed_store_persistence_contract.py"], "impact": "Contrato de persistência lógica e física alinhado.", "risk": "Controlado por teste de completude e rollback.", "proposal": "Manter o gate em toda inclusão de entidade ou store.", "dependencies": [], "affected_files": ["modules/shared/*_postgres_store.py", "database/postgres/migrations"], "migration": "57 relações implementadas em duas migrations reversíveis", "backend": "120 de 120 entidades com destino físico tipado", "frontend": "120 de 120 entidades com superfície canônica", "tests": "contrato de destino tipado, aliases e reversibilidade aprovados", "documentation": "02_MAPA_DE_DOMINIOS.md", "acceptance": "Cada entidade possui decisão explícita de persistência, ownership e coordenada UI, ou justificativa versionada de ausência.", "status": "resolvido", "owner_suggestion": "arquitetura e responsáveis de domínio", "dimensions": ["tabelas_colecoes", "campos", "relacionamentos", "formularios"]
        },
        {
            "id": "AUD-P1-007", "priority": "P1", "module": "auditoria e segurança", "title": "Contrato unificado de auditoria ainda exige homologação operacional por módulo",
            "description": "A migration 029 e o writer compartilhado cobrem estruturalmente os 35 requisitos, são usados pela base e pelos sete stores PostgreSQL especializados e capturam contexto HTTP governado. Leituras sensíveis registram finalidade, exportação, impressão e compartilhamento. A aplicação da migration em PostgreSQL vivo e a homologação positiva e negativa por operação sensível ainda não estão comprovadas.", "evidence": ["database/postgres/migrations/029_unified_immutable_audit.sql", "modules/shared/audit_contract.py", "modules/shared/store.py", "modules/shared/postgres_store.py", "modules/shared/runtime.py", "tests/test_audit_contract.py", "tests/test_permissions_enforcement_matrix.py", "docs/data-audit/artifacts/cobertura_auditoria.json"], "impact": "Sem PostgreSQL vivo e matriz operacional, a integridade e o acesso da trilha não estão homologados ponta a ponta.", "risk": "Não repúdio e comprovação LGPD ainda dependem de validação operacional.", "proposal": "Aplicar a migration em homologação e executar matriz positiva e negativa de mutação, leitura, exportação, impressão e compartilhamento por módulo sensível.", "dependencies": ["PostgreSQL de homologação", "segurança", "compliance", "proprietários de domínio"], "affected_files": ["database/postgres/migrations/029_unified_immutable_audit.sql", "tests/test_postgres_priority_stores_integration.py", "modules"], "migration": "029 implementada e reversível; aplicação em PostgreSQL vivo pendente", "backend": "writer único, stores especializados, contexto HTTP e leitura sensível implementados", "frontend": "enviar motivo quando obrigatório e nunca registrar segredo", "tests": "contrato, integridade, minimização, contexto HTTP e leitura sensível aprovados; PostgreSQL vivo por operação pendente", "documentation": "08_AUDITORIA_E_LOGS.md", "acceptance": "Os 35 requisitos de auditoria têm implementação ou decisão justificada por operação, com integridade, retenção e testes aprovados.", "status": "implementacao_parcial", "owner_suggestion": "segurança, plataforma e compliance", "dimensions": ["auditoria", "campos_sensiveis", "permissoes_backend"]
        },
        {
