...
def build_delivery() -> None:
    ...
    gaps = [
        ...
        },
        {
            ...
            "description": "A migration 028 e o serviço dynamic_forms implementam catálogo, bindings, versões, blueprint, cálculos allowlist, homologação segregada, publicação com MFA, submissões idempotentes, eventos e cobrança; builder web e homologação PostgreSQL operacional permanecem pendentes.", "evidence": ["database/postgres/migrations/028_dynamic_forms_governance.sql", "modules/dynamic_forms/main.py", "modules/shared/dynamic_forms.py", "modules/shared/dynamic_forms_postgres_store.py", "tests/test_dynamic_forms_api.py", "tests/test_dynamic_forms_domain.py"], "impact": "Sem UI e validação em PostgreSQL vivo, o produto ainda não pode ser homologado ponta a ponta.", "risk": "Experiência indisponível ao usuário empresarial e comportamento SQL ainda não comprovado em ambiente real.", "proposal": "Implementar builder allowlist e prévia web; executar migration e fluxos completos em PostgreSQL homologado.", "dependencies": ["builder web", "PostgreSQL de homologação"], "affected_files": ["apps/all-in-one/src", "modules/dynamic_forms", "database/postgres/migrations/028_dynamic_forms_governance.sql"], "migration": "implementada e reversível", "backend": "APIs, RBAC/ABAC, homologação, publicação, submissão e eventos implementados", "frontend": "implementar builder, preview e publicação", "tests": "domínio, segurança HTTP, tenant, MFA e idempotência aprovados; PostgreSQL vivo pendente", "documentation": "10_FORMULARIOS_DINAMICOS.md", "acceptance": "Metadados, API, homologação, segurança, publicação, cobrança, auditoria e testes estão implementados.", "status": "implementacao_parcial", "owner_suggestion": "produto, arquitetura e engenharia", "dimensions": ["tabelas_colecoes", "campos", "formularios", "acoes_ui", "permissoes_backend", "auditoria", "calculos"]
        },
        {
