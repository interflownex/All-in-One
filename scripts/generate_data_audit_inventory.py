...
def build_delivery() -> None:
    ...
    gaps = [
        ...
        {
            "id": "AUD-P1-007", "priority": "P1", "module": "auditoria e segurança", "title": "Contrato unificado de auditoria ainda exige adoção e homologação por operação",
            "description": "A migration 029 e o contrato compartilhado cobrem estruturalmente os 35 requisitos de alteração e leitura, com append-only, cadeia SHA-256, retenção, minimização de segredos e suporte a leitura, exportação, impressão e compartilhamento. A adoção nos stores especializados, a captura integral do contexto HTTP e a homologação por operação sensível ainda não estão comprovadas.", "evidence": ["database/postgres/migrations/029_unified_immutable_audit.sql", "modules/shared/audit_contract.py", "modules/shared/store.py", "modules/shared/postgres_store.py", "tests/test_audit_contract.py", "docs/data-audit/artifacts/cobertura_auditoria.json"], "impact": "Operações fora dos stores compartilhados podem continuar gerando evidência incompleta.", "risk": "Não repúdio e comprovação LGPD ainda desiguais entre módulos.", "proposal": "Adotar o writer unificado em cada store especializado, capturar contexto HTTP permitido e homologar leitura/mutação sensível por operação.", "dependencies": ["segurança", "compliance", "proprietários de domínio"], "affected_files": ["modules/shared/*_postgres_store.py", "modules/shared/runtime.py", "modules"], "migration": "029 implementada e reversível; aplicação em PostgreSQL vivo pendente", "backend": "contrato base e SQLite implementados; stores especializados e contexto HTTP pendentes", "frontend": "enviar motivo quando obrigatório e nunca registrar segredo", "tests": "contrato, integridade, minimização, leitura sensível e rollback aprovados; integração por operação pendente", "documentation": "08_AUDITORIA_E_LOGS.md", "acceptance": "Os 35 requisitos de auditoria têm implementação ou decisão justificada por operação, com integridade, retenção e testes aprovados.", "status": "implementacao_parcial", "owner_suggestion": "segurança, plataforma e compliance", "dimensions": ["auditoria", "campos_sensiveis", "permissoes_backend"]
        },
        {
